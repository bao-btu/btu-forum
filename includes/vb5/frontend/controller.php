...
class vB5_Frontend_Controller
{
	...
	public function showMsgPage($title, $msg)
	{
		...
		if (!vB5_Request::get('useEarlyFlush'))
		{
			$output = $preheader . $output;
		}
		$this->outputPage($output);
	}
	...
	protected function parseBbCodeForPreview($rawText)
	{
		$results = array();
		...
		try
		{
			$results['parsedText'] = vB5_Frontend_Controller_Bbcode::parseWysiwygForPreview($rawText);
		}
		catch (Exception $e)
		{
