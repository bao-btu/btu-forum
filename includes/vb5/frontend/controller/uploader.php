...
class vB5_Frontend_Controller_Uploader extends vB5_Frontend_Controller
{
	...
	public function actionUrl()
	{
		if (isset($_REQUEST['urlupload']))
		{
			...
			$response = $api->callApi('content_attach', 'uploadUrl', array('url' => $_REQUEST['urlupload'], 'attachment' => $_REQUEST['attachment']));
			...
			$response['thumbUrl'] = $config->baseurl . '/filedata/fetch?filedataid=' . $response['filedataid'] . '&type=thumb';
	...
	**/
	...
	public function actionUploadPhoto()
	{
		if ($_FILES AND !empty($_FILES) )
		{
			...
			if (!empty($response['filedataid']))
			{
				...
				$imgUrl = $config->baseurl . '/filedata/fetch?filedataid=' . $response['filedataid'] . "&type=thumb";
	...
	public function actionUploadSGIcon()
	{
		...
		$response['thumbUrl'] = $config->baseurl . '/filedata/fetch?filedataid=' . $filedataid . '&type=thumb';
		...
		if ($file->name)
		{
			...
			else
			{
				if ($file_size === $file->size)
				{
					...
					$file->thumbnail_url = $this->baseurl . '/filedata/fetch?filedataid=' . $fileInfo['filedataid'] . '&type=thumb' ;
				}
				...
				if (isset($fileInfo))
				{
					...
					$this->fileData[$name]['thumbnail_url'] = $this->baseurl . '/filedata/fetch?filedataid=' . $fileInfo['filedataid'] . '&type=thumb' ;
