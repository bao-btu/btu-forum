...
class vB5_Template_Stylesheet
{
	...
	public function insertCss(&$content, $isAjaxTemplateRender)
	{
		...
		if ($storecssasfile)
		{
			...
			{
				$replace .= '<link rel="stylesheet" type="text/css" href="' . htmlspecialchars($vbcsspath . $cssfiledate . '-' . $css) . "\" />\n";
				$replaceAjax[] = htmlspecialchars($vbcsspath . $css);
			}
		}
		else
		{
