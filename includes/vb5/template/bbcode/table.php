...
* @package	vBulletin
*/
class vB5_Template_BbCode_Table
{
	/**
	...
	* @var	string
	*/
	protected $table_class_prefix = 'text_table_';
	...
	*/
	public function __construct(vB5_Template_Bbcode $parser)
	{
		$this->parser = $parser;
	}
	...
	*/
	public function parseTableTag($content, $params = '')
	{
		...
		if (!isset($table_params['attributes']['class']) OR !$table_params['attributes']['class'])
		{
		...
		if ($this->wrap_table)
		{
			$table = "<div class=\"" . $this->table_class_prefix . "\">$table</div>";
		}
	...
	*/
	protected function parseTableRows($content, array $table_classes)
	{
		...
		foreach ($rows AS $row)
		{
			...
			if (!isset($row_params['attributes']['class']) OR !$row_params['attributes']['class'])
			{
	...
	*/
	protected function parseTableCells($content, array $row_classes)
	{
		...
		foreach ($cells AS $cell)
		{
			...
			if (!isset($cell_params['attributes']['class']) OR !$cell_params['attributes']['class'])
			{
	...
	*/
	protected function modifyCellContent($content)
	{
		...
		return $this->parser->stripFrontBackWhitespace($content, 1);
	}
