...
*
* @package 		vBulletin
* @version		$Revision: 75396 $
* @date 		$Date: 2013-05-29 14:33:37 -0700 (Wed, 29 May 2013) $
*
*/
	...
	*/
	protected function &cacheSmilies($do_html)
	{
		...
		foreach (self::$smilies AS $smilie)
		{
			...
			{
				$smiliepath = preg_match('#^https?://#si', $smilie['smiliepath']) ? $smilie['smiliepath'] : vB5_Config::instance()->baseurl_core . '/' . $smilie['smiliepath'];
						...
						if ((isset($tag_info['strip_empty']) AND $tag_info['strip_empty'] == false) OR trim($open['data']) != '')
						{
							...
							if (empty($tag_info['data_regex']) OR preg_match($tag_info['data_regex'], $open['data']))
							{
								...
								if (isset($tag_info['html']))
								{
									...
									$pending_text = sprintf($tag_info['html'], $open['data'], $open['option'], vB5_Config::instance()->baseurl);
								}
...
//		global $vbphrase, $show;
		...
		{
			$regexfind[] = '#<((?>[^>"\']+?|"[^"]*"|\'[^\']*\')+)>#e';
			$regexreplace[] = "\$this->handle_bbcode_html_tag(vB5_String::htmlSpecialCharsUni(str_replace('\\\"', '\"', '\\1')))";
		}
		// parse the code
	...
	*/
	function handle_bbcode_img($bbcode, $do_imgcode, $has_img_code = false, $fulltext = '', $forceShowImages = false)
	{
		...
		if (($has_img_code & self::BBCODE_HAS_ATTACH) AND preg_match_all('#\[attach(?:=(right|left|config))?\]([[:alnum:]]+)\[/attach\]#i', $fulltext, $matches))
		{
			...
			if (!empty($legacyIds))
			{
				...
				{
					$this->setAttachments($this->oldAttachments);
				}
			}
	...
	protected function attachReplaceCallback($matches)
	{
		...
		else
		{
			...
			{
				$attachmentid =  $this->oldAttachments[intval($matches[2])]['filedataid'];
		...
		{	// attachment specified by [attach] tag belongs to this post
			...
			$forceimage = false;
			if ($cangetattachment AND $canseethumbnails AND isset($attachment['resize_filesize']) AND ($attachment['resize_filesize'] == $attachment['filesize']))
			{
...
//						}
					...
					if (($settings OR (self::$viewAttachedImages == 1 AND !empty($attachment['hasthumbnail']))) AND vB5_User::get('showimages') AND $cangetattachment)
					{
						if (empty($link))
						{
							$link = self::$config->baseurl . '/filedata/fetch?';
						}
						...
						$hrefbits = array(
							...
							'id'     => $id,
							'class'     => 'bbcode-attachment galleryLink',
						);
						if ($lightbox)
						{
...
//						}
						...
						$imgbits = array(
							...
							'border' => '0',
							'alt'    => $alt_text ? $alt_text : vB5_Template_Phrase::instance()->register(array('image_larger_version_x_y_z', $attachment['filename'], $attachment['counter'], $attachment['filesize'], $attachment['filedataid']))
						);
					...
					else if ($showOldImage OR
						(($forceimage OR self::$viewAttachedImages == 3) AND !in_array($attachment['extension'], array('tiff', 'tif', 'psd', 'pdf')) AND $cangetattachment))
					{	// Display the attachment with no link to bigger image
						...
						. vB5_Template_Phrase::instance()->register(array('image_x_y_z', $attachment['filename'], $attachment['counter'], $attachment['filesize']))
		...
		{	// Belongs to another post so we know nothing about it ... or we are not displying images so always show a link
			return "<a href=\"" . self::$config->baseurl . "/filedata/fetch?filedataid=$attachmentid\">" . vB5_Template_Phrase::instance()->register(array('attachment')) . " </a>";
		}
	}
	...
	*/
	public function append_noninline_attachments($text, $attachments, $do_imgcode = false)
	{
		if (!empty($attachments))
		{
			...
			$attach_url = self::$config->baseurl . "/filedata/fetch?id=";
			if ($this->renderImmediate)
			{
