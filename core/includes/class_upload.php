...
*
* @package	vBulletin
* @version	$Revision: 72714 $
* @date		$Date: 2013-03-12 10:29:36 -0700 (Tue, 12 Mar 2013) $
*/
...
class vB_Upload_Attachment extends vB_Upload_Abstract
{
	...
	function process_upload($uploadstuff = '', $forcethumbnail = false, $imageonly = false)
	{
		...
		if (is_array($uploadstuff) AND $uploadstuff['filedataid'])
		{
			...
			$attachdata =& datamanager_init('Attachment', $this->registry, vB_DataManager_Constants::ERRTYPE_SILENT, 'attachment');
		...
		else if ($this->accept_upload($uploadstuff))
		{
			...
			if (!$this->is_valid_extension($this->upload['extension']) OR (!$this->image->isValidInfoExtension($this->upload['extension']) AND $imageonly))
			{
				$this->set_error('upload_invalid_file');
				return false;
			}
			...
			if ($this->image->isValidInfoExtension($this->upload['extension']))
			{
				...
				if ($this->imginfo = $this->image->fetchImageInfo($this->upload['location']))
				{
					...
					if (($this->maxwidth > 0 AND $this->imginfo[0] > $this->maxwidth) OR ($this->maxheight > 0 AND $this->imginfo[1] > $this->maxheight))
					{
						...
						if ($this->registry->options['attachresize'] AND $this->image->isValidResizeType($this->imginfo[2]))
						{
							$this->upload['resized'] = $this->image->fetchThumbnail($this->upload['filename'], $this->upload['location'], $this->maxwidth, $this->maxheight, $this->registry->options['thumbquality'], false, false, true, false);
							if (empty($this->upload['resized']['filedata']))
							{
								if (!empty($this->upload['resized']['imageerror']) AND $this->registry->userinfo['permissions']['adminpermissions'] & $this->registry->bf_ugp_adminpermissions['cancontrolpanel'])
								{
		...
		if ($this->registry->options['attachtotalspace'])
		{
			...
			if (($filedata['sum'] + $filesize) > $this->registry->options['attachtotalspace'])
			{
				...
				$maildata = vB_Api::instanceInternal('phrase')->
						fetchEmailPhrases('attachfull', array($vbulletin->userinfo['username'], $vbulletin->options['attachtotalspace'], $vbulletin->options['bburl'], $admincpdir), array($vbulletin->options['bbtitle']), 0);
...
class vB_Upload_SocialGroupIcon extends vB_Upload_Abstract
{
	...
	function process_upload($uploadurl = '')
	{
		...
		if ($this->accept_upload($uploadstuff))
		{
			if ($this->imginfo = $this->image->fetchImageInfo($this->upload['location']))
			{
				...
				if (($this->imginfo[0] > FIXED_SIZE_GROUP_ICON_WIDTH) OR ($this->imginfo[1] > FIXED_SIZE_GROUP_ICON_HEIGHT) OR $this->image->fetchMustConvert($this->imginfo[2]))
				{
					// shrink-a-dink a big fat image or an invalid image for browser display (PSD, BMP, etc)
					$this->upload['resized'] = $this->image->fetchThumbnail($this->upload['filename'], $this->upload['location'], FIXED_SIZE_GROUP_ICON_WIDTH, FIXED_SIZE_GROUP_ICON_HEIGHT, $this->registry->options['thumbquality'], false, false, false, false);
					if (empty($this->upload['resized']['filedata']))
					{
						$this->set_error('upload_exceeds_dimensions', $this->maxwidth, $this->maxheight, $this->imginfo[0], $this->imginfo[1]);
						return false;
	...
	*/
	...
	{
		$this->upload['thumbnail_filedata'] = $this->image->fetchThumbnail(
...
/*======================================================================*\
|| ####################################################################
|| # CVS: $RCSfile: class_upload.php,v $ - $Revision: 72714 $
|| ####################################################################
\*======================================================================*/
