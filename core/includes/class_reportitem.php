...
class vB_ReportItem
{
	...
	function do_report($reason, &$iteminfo)
	{
		...
		if ($reportthread)
		{
				...
				))
			{
				...
				$threadman =& datamanager_init('Thread_FirstPost', $this->registry, vB_DataManager_Constants::ERRTYPE_SILENT, 'threadpost');
				...
				if ($rpthreadid = $threadman->save())
				{
					...
					else
					{
						...
						{
							$threadman =& datamanager_init('Thread', $this->registry, vB_DataManager_Constants::ERRTYPE_SILENT, 'threadpost');
				...
				$rpthreadinfo['forumid'] == $rpforuminfo['forumid'])
			{
				...
				$maildata = vB_Api::instanceInternal('phrase')
					->fetchEmailPhrases('reportitem_newpost', array($reportinfo['reporterlink'], $reportinfo['rusername'], $reportinfo['reason']), array());
				...
				$postman =& datamanager_init('Post', $this->registry, vB_DataManager_Constants::ERRTYPE_STANDARD, 'threadpost');
				...
				$postman->set('title', $maildata['subject']);
				$postman->set('pagetext', $maildata['message']);
				if ($userinfo['userid'] != $this->registry->userinfo['userid'])
				{
					// not posting as the current user, IP won't make sense
		...
		if ($reportemail)
		{
			...
			// Legacy Hook 'report_send_process' Removed //
			...
			// Legacy Hook 'report_send_complete' Removed //
		}
	}
	...
	function send_moderator_email($moderator, $rpthreadinfo, $reportinfo)
	{
		...
		// Legacy Hook 'report_send_email' Removed //
...
//		$reportinfo['reporterlink'] = fetch_seo_url('member|js|nosession|bburl', $reportinfo, null, 'ruserid', 'rusername');
		$reportinfo['reporterlink'] = vB5_Route::buildUrl('profile|js|nosession|bburl', $reportinfo);
...
//				$reportinfo, null, 'puserid', 'pusername');
			$reportinfo['posterlink'] = vB5_Route::buildUrl('profile|js|nosession|bburl', $reportinfo);
		}
	}
...
class vB_ReportItem_Post extends vB_ReportItem
{
	...
	function fetch_affected_moderators()
	{
		return $this->registry->db->query_read_slave("
			...
			WHERE moderator.nodeid IN (" . $this->extrainfo['forum']['parentlist'] . ")
				AND moderator.nodeid <> -1
		");
	}
	...
	function set_reportinfo(&$reportinfo)
	{
		...
		$reportinfo['prefix_plain'] = '';
		if($this->extrainfo['thread']['prefixid'])
		{
			$phraseAux = vB_Api::instanceInternal('phrase')->fetch(array("prefix_" . $this->extrainfo['thread']['prefixid'] . "_title_plain"));
			$reportinfo['prefix_plain'] = $phraseAux["prefix_" . $this->extrainfo['thread']['prefixid'] . "_title_plain"];
		}
	}
	...
	function update_item_reportid($newthreadid)
	{
		$postman =& datamanager_init('Post', $this->registry, vB_DataManager_Constants::ERRTYPE_SILENT, 'threadpost');
		$postman->set_info('is_automated', true);
		$postman->set_info('parseurl', true);
...
class vB_ReportItem_VisitorMessage extends vB_ReportItem
{
	...
	function set_forminfo(&$iteminfo)
	{
		...
		$this->forminfo = array(
			...
			'itemlink'     => vB5_Route::buildUrl('profile', $this->extrainfo['user']),
		);
...
//				)
//			),
			'memberlink_pi' => vB5_Route::buildUrl('profile|js|nosession|bburl', $profile_user),
//			'memberlink'    => fetch_seo_url('member|js|nosesson|bburl',$profile_user),
			'memberlink'    => vB5_Route::buildUrl('profile|js|nosession|bburl',$profile_user),
		));
	}
	...
	function update_item_reportid($newthreadid)
	{
		$dataman =& datamanager_init('VisitorMessage', $this->registry, vB_DataManager_Constants::ERRTYPE_SILENT);
...
class vB_ReportItem_GroupMessage extends vB_ReportItem
{
	...
	function set_forminfo(&$iteminfo)
	{
		...
		$this->forminfo = array(
			...
			'reporttype'   => $vbphrase['social_group_guser'],
			'description'  => $vbphrase['only_used_to_report'],
	...
	function update_item_reportid($newthreadid)
	{
		$dataman =& datamanager_init('GroupMessage', $this->registry, vB_DataManager_Constants::ERRTYPE_SILENT);
		$dataman->set_info('is_automated', true);
		$dataman->set_info('parseurl', true);
		$dataman->set('reportthreadid', $newthreadid);
...
class vB_ReportItem_AlbumPicture extends vB_ReportItem
{
	...
	function set_forminfo(&$iteminfo)
	{
		...
		$this->forminfo = array(
			...
			'reporttype'   => $vbphrase['album_guser'],
			'description'  => $vbphrase['only_report_inappropriate_pictures'],
			'itemname'     => $this->extrainfo['album']['title'],
			'itemlink'     => "album.php?" . vB::getCurrentSession()->get('sessionurl') . "albumid=" . $this->extrainfo['album']['albumid'],
		);
...
class vB_ReportItem_PictureComment extends vB_ReportItem
{
	...
	function set_forminfo(&$iteminfo)
	{
		...
		$this->forminfo = array(
			...
			'reporttype'   => ($this->extrainfo['picture']['albumid'] ? $vbphrase['album_guser'] : $vbphrase['social_group_guser']),
			'description'  => $vbphrase['only_used_to_report'],
			...
			'itemlink'     => ($this->extrainfo['picture']['albumid']
				? "album.php?" . vB::getCurrentSession()->get('sessionurl') . "albumid=" . $this->extrainfo['picture']['albumid']
