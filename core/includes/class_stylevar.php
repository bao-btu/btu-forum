...
*
* @package	vBulletin
* @version	$Revision: 70095 $
* @date		$Date: 2013-01-04 13:00:47 -0800 (Fri, 04 Jan 2013) $
*/
...
abstract class vB_StyleVar
{
	...
	public function print_editor()
	{
		...
		if ($this->inherited == -1)
		{
			...
			$header .= (count($header) == 1 ? '<span class="smallfont">' : '') . construct_link_code($vbphrase['revert_gcpglobal'], "stylevar.php?" . vB::getCurrentSession()->get('sessionurl') . "do=confirmrevert&amp;dostyleid=" . $vbulletin->GPC['dostyleid'] . "&stylevarid=" . $this->stylevarid . "&rootstyle=-1");
		}
	...
	protected function print_color_input_row($title, $name, $value)
	{
		...
		if (self::$need_colorpicker)
		{
			...
			$cp .= '
					...
					var cleargifurl = "' . vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'] .'";
					//-->
				</script>';
		...
		$title_attr = ($vb5_config['Misc']['debug'] ? " title=\"name=&quot;$name&quot;\"" : '');
		$cell =
	...
	protected function print_background_output()
	{
		...
		{
			 /* this assumes that the images folder is stored in root directory and that the core directory is called "core" */
			$image = str_replace('(', '(../core/', $image);
		}
...
class vB_StyleVar_default extends vB_StyleVar
{
	...
	public function print_editor_form()
	{
		...
		switch ($this->datatype)
		{
			...
			case 'url':
				print_input_row($vbphrase['url_gstyle'], 'stylevar[' . $this->stylevarid . ']', $this->value[$this->datatype], true);
			break;
			...
			case 'size':
				...
				print_input_row($vbphrase['size_gstyle'], 'stylevar[' . $this->stylevarid . '][size]', $this->value['size'], true);
			break;
...
class vB_StyleVar_padding extends vB_StyleVar
{
	public function print_editor_form()
	{
		...
		print_input_row($vbphrase['right_gstyle'], 'stylevar[' . $this->stylevarid . '][right]', $this->value['right'], true);
		print_input_row($vbphrase['bottom'], 'stylevar[' . $this->stylevarid . '][bottom]', $this->value['bottom'], true);
		print_input_row($vbphrase['left_gstyle'], 'stylevar[' . $this->stylevarid . '][left]', $this->value['left'], true);
	}
}
...
class vB_StyleVar_textdecoration extends vB_StyleVar
{
	public function print_editor_form()
	{
		...
		print_yes_no_row($vbphrase['underline_gstyle'], 'stylevar[' . $this->stylevarid . '][underline]', $this->value['underline']);
...
class vB_StyleVar_font extends vB_StyleVar
{
	public function print_editor_form()
	{
		...
		print_input_row($vbphrase['font_family_gstyle'], 'stylevar[' . $this->stylevarid . '][family]', $this->value['family'], true);
...
class vB_StyleVar_border extends vB_StyleVar
{
	public function print_editor_form()
	{
		...
		$this->print_color_input_row($vbphrase['color_gstyle'], 'stylevar[' . $this->stylevarid . '][color]', $this->value['color']);
	}
}
