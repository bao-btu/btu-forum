...
function construct_ip_usage_table($ipaddress, $prevuserid, $depth = 1)
{
	...
	$retdata = '';
	foreach ($users['postipusers'] AS $user)
	{
		$retdata .= '<li>' .
			"<a href=\"user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=" . iif(VB_AREA == 'ModCP', 'viewuser', 'edit') . "&amp;u=$user[userid]\"><b>$user[username]</b></a> &nbsp;
			<a href=\"$userscript?" . vB::getCurrentSession()->get('sessionurl') . "do=gethost&amp;ip=$user[ipaddress]\" title=\"" . $vbphrase['resolve_address'] . "\">$user[ipaddress]</a> &nbsp; " .
			construct_link_code($vbphrase['find_posts_by_user'], "../search.php?" . vB::getCurrentSession()->get('sessionurl') . "do=finduser&amp;u=$user[userid]", '_blank') .
			construct_link_code($vbphrase['view_other_ip_addresses_for_this_user'], "$userscript?" . vB::getCurrentSession()->get('sessionurl') . "do=doips&amp;u=$user[userid]&amp;hash=" . CP_SESSIONHASH) .
			"</li>\n";
...
function construct_ip_register_table($ipaddress, $prevuserid, $depth = 1)
{
	...
	$retdata = '';
	if ($users = @array_pop($users['regipusers']))
	{
...
function construct_user_ip_table($userid, $previpaddress, $depth = 2)
{
	...
	{
		$retdata .= '<li>' .
			"<a href=\"$userscript?" . vB::getCurrentSession()->get('sessionurl') . "do=gethost&amp;ip=$ip[ipaddress]\" title=\"" . $vbphrase['resolve_address'] . "\">$ip[ipaddress]</a> &nbsp; " .
			construct_link_code($vbphrase['find_more_users_with_this_ip_address'], "$userscript?" . vB::getCurrentSession()->get('sessionurl') . "do=doips&amp;ipaddress=$ip[ipaddress]&amp;hash=" . CP_SESSIONHASH) .
			"</li>\n";
...
function print_user_search_rows($email = false)
{
	...
	print_label_row($vbphrase['username'], "
		...
		/><input type=\"image\" value=\"\" src=\"" . vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'] . "\" width=\"1\" height=\"1\"
