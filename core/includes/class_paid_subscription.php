...
*
* @package	vBulletin
* @version	$Revision: 74513 $
* @date		$Date: 2013-04-25 16:01:32 -0700 (Thu, 25 Apr 2013) $
*
* @abstract
*
*/
...
//			$countaccess = vB::getDbAssertor()->getRow('masks', array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_COUNT, 'userid' => $userid));
			...
			if($sub['nusergroupid'] == $user['usergroupid'] AND $user['usergroupid'] != $user['pusergroupid'])
			{
				...
				if (!empty($subids))
				{
					$activesub = vB::getDbAssertor()->getRow('subscriptionlog', array('userid' => $userid, 'subscriptionid' => $subids), array('field' => 'expirydate', 'direction' => vB_dB_Query::SORT_DESC));
				}
				if ($activesub)
				{
					// there is at least one active subscription with the same primary usergroup, so alter its resetgroup
					vB::getDbAssertor()->update('subscriptionlog', array('pusergroupid' => $user['pusergroupid']), array('subscriptionlogid' => $activesub['subscriptionlogid']));
			...
			{ // they need a default title
				...
				{ // should be a title based on minposts it seems then
					$usergroup = vB::getDbAssertor()->getRow('usertitle', array(
							array('field'=> 'minposts', 'value' => $user[posts], vB_Db_Query::OPERATOR_KEY => vB_Db_Query::OPERATOR_LTE)
