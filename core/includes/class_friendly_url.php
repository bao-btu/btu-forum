...
abstract class vB_Friendly_Url
{
	...
	protected function get_query()
	{
		...
		{
			$arguments['s'] = vB::getCurrentSession()->get('dbsessionhash');
		}
	...
	protected function consume_request_pageinfo($clean = false)
	{
		...
		foreach ($query AS $key => $value)
		{
			if (!$this->skip_query_var($key))
			{
				if ($clean)
				{
					$key = $this->registry->cleaner->xssClean($key);
					$value = $this->registry->cleaner->xssClean($value);
				}
	...
	public function redirect_canonical_url($request_uri)
	{
		...
		// Legacy Hook 'friendlyurl_redirect_canonical' Removed //
