...
function verify_groupmessage($gmid, $alert = true, $perm_check = true)
{
	...
	if (!$messageinfo)
	{
		if ($alert)
		{
			standard_error(fetch_error('invalidid', $vbphrase['social_group_gsocialgroups']));
		}
		else
		{
			return 0;
	...
	if ($perm_check)
	{
		if ($messageinfo['state'] == 'deleted')
		{
			...
			if (!$can_view_deleted)
			{
				standard_error(fetch_error('invalidid', $vbphrase['message']));
			}
		}
...
function verify_socialdiscussion($discussionid, $alert = true, $perm_check = true)
{
	...
	if (!($discussion = fetch_socialdiscussioninfo($discussionid)) OR !isset($discussion['groupid']))
	{
		if ($alert)
		{
			standard_error(fetch_error('invalidid', $vbphrase['social_group_discussion']));
		}
...
function show_group_inlinemoderation($group, &$show, $bdiscussions)
{
	...
	{
		$vbphrase['delete_messages_js'] = addslashes_js($vbphrase['delete_messages_ginlinemod']);
...
function fetch_socialgroup_picture($attachmentid, $groupid)
{
	...
	// Legacy Hook 'group_fetch_pictureinfo' Removed //
...
function build_group_counters($groupid)
{
	...
	$dataman =& datamanager_init('SocialGroup', $vbulletin, vB_DataManager_Constants::ERRTYPE_SILENT);
	...
	// Legacy Hook 'group_build_counters' Removed //
...
function exec_send_sg_notification($discussionid, $gmid = false, $postusername = false)
{
	...
	// Legacy Hook 'newpost_sg_notification_start' Removed //
	...
	while ($touser = $vbulletin->db->fetch_array($useremails))
	{
		...
		$touser['auth'] = md5($touser['userid'] . $touser['subscribediscussionid'] . $touser['salt'] . vB_Request_Web::COOKIE_SALT);
		...
		// Legacy Hook 'new_sg_message_notification_message' Removed //
...
function fetch_owner_socialgroup($userid, $page = 0)
{
	...
	// Legacy Hook 'group_fetch_own' Removed //
...
function fetch_socialgroup_random_groups()
{
	...
	// Legacy Hook 'group_fetch_random' Removed //
