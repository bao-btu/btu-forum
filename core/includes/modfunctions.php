	...
	foreach($vbulletin->forumcache AS $forum)
	{
		...
		if (empty($forum['link']))
		{
			if ($permcheck == 'none' OR can_moderate($forum['forumid'], $permcheck))
			{
				...
				if ($show_no_posting)
				{
					$select_options["$forum[forumid]"] .= ' ' . ($forum['options'] & $vbulletin->bf_misc_forumoptions['allowposting'] ? '' : " ($vbphrase[no_posting_gthreadmanage])") . " $forum[allowposting]";
				}
			}
		}
	}
...
function show_inline_mod_login($showerror = false)
{
	...
	eval(standard_error(fetch_error('nopermission_loggedin',
		...
		vB5_Route::buildUrl($forumHome['routeid'] . 'home|nosession|fullurl')
	)));
