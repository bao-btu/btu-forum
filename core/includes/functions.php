...
{
	if (function_exists('mb_strtolower') AND $newstring = @mb_strtolower($string, getTempCharSet()))
	{
		return $newstring;
	}
	else
...
{
	$hash = sha1(vB::getRequest()->getTimeNow() . SESSION_HOST . microtime() . uniqid(mt_rand(), true) . @implode('', @fstat(@fopen( __FILE__, 'r'))));
...
function exec_mail_queue()
{
	...
	if ($vbulletin->mailqueue !== null AND $vbulletin->mailqueue > 0 AND $options['usemailqueue'])
	{
...
function vbmail($toemail, $subject, $message, $sendnow = false, $from = '', $uheaders = '', $username = '')
{
	...
	if (!($mail = vB_Mail::fetchLibrary($vbulletin, !$sendnow AND $options['usemailqueue'])))
	{
		return false;
	}
...
function fetch_foruminfo(&$forumid, $usecache = true)
{
	...
	if (!$usecache OR !isset($vbulletin->forumcache["$forumid"]))
	{
		...
		$vbulletin->forumcache["$forumid"] = $vbulletin->db->query_first_slave("
			SELECT forum.*
			FROM " . TABLE_PREFIX . "node AS forum
			WHERE forum.nodeid = $forumid
		");
	}
	...
	// Legacy Hook 'fetch_foruminfo' Removed //
...
function fetch_threadinfo(&$threadid, $usecache = true)
{
	...
	{
		// Legacy Hook 'fetch_threadinfo_query' Removed //

		$marking = ($options['threadmarking'] AND $vbulletin->userinfo['userid']);
		$threadcache["$threadid"] = $vbulletin->db->query_first("
			...
			" . iif($vbulletin->userinfo['userid'] AND ($options['threadsubscribed'] AND THIS_SCRIPT == 'showthread') OR THIS_SCRIPT == 'editpost' OR THIS_SCRIPT == 'newreply' OR THIS_SCRIPT == 'postings', 'NOT ISNULL(subscribethread.subscribethreadid) AS issubscribed, emailupdate, folderid,')
			  . iif($options['threadvoted'] AND $vbulletin->userinfo['userid'], 'threadrate.vote,')
	...
	// Legacy Hook 'fetch_threadinfo' Removed //
...
function fetch_postinfo(&$postid)
{
	...
	{
		// Legacy Hook 'fetch_postinfo_query' Removed //
	...
	// Legacy Hook 'fetch_postinfo' Removed //
...
*/
function fetch_userinfo(&$userid, $option = 0, $languageid = 0, $nocache = 0)
{
	...
	// Legacy Hook 'fetch_userinfo_query' Removed //
	...
	$user = $vbulletin->db->query_first_slave("
		SELECT " .
			...
			iif(($option & FETCH_USERINFO_AVATAR) AND $options['avatarenabled'], ', avatar.avatarpath, NOT ISNULL(customavatar.userid) AS hascustomavatar, customavatar.dateline AS avatardateline, customavatar.width AS avwidth, customavatar.height AS avheight, customavatar.height_thumb AS avheight_thumb, customavatar.width_thumb AS avwidth_thumb, customavatar.filedata_thumb').
		...
		iif(($option & FETCH_USERINFO_AVATAR) AND $options['avatarenabled'], "LEFT JOIN " . TABLE_PREFIX . "avatar AS avatar ON (avatar.avatarid = user.avatarid) LEFT JOIN " . TABLE_PREFIX . "customavatar AS customavatar ON (customavatar.userid = user.userid) ") .
		...
		iif(!isset($vbphrase), "LEFT JOIN " . TABLE_PREFIX . "language AS language ON (language.languageid = " . (!empty($languageid) ? $languageid : "IF(user.languageid = 0, " . intval($options['languageid']) . ", user.languageid)") . ") ") .
		(($vbulletin->userinfo['userid'] AND ($option & FETCH_USERINFO_ISFRIEND)) ?
	...
	// Legacy Hook 'fetch_userinfo' Removed //
	...
	{ // consider showing an error message if they're not sent using above variables?
		...
		if ($value === '' OR $value === false)
		{
			...
			$scriptpath = preg_replace(
				array(
					'#/[^/]+\.php$#i',
					'#/(' . preg_quote($vb5_config['Misc']['admincpdir'], '#') . '|' . preg_quote($vb5_config['Misc']['modcpdir'], '#') . ')(/|$)#i'
				),
				'',
				$scriptpath
			);
...
function fetch_email_phrases($email_phrase, $languageid = -1, $emailsub_phrase = '', $varprefix = '')
{
	...
	{
		$emailsub_phrase .= '_gemailsubject';
	}
	...
	$phraseArrayAux = vB_Api::instanceInternal('phrase')->fetch(array($email_phrase,$emailsub_phrase));
	return
		'$' . $varprefix . 'message = "' . $phraseArrayAux[$email_phrase] . '";' .
		'$' . $varprefix . 'subject = "' . $phraseArrayAux[$emailsub_phrase] . '";';
}
...
function cache_ordered_forums($getcounters = 0, $getinvisibles = 0, $userid = 0)
{
	...
	if ($getcounters)
	{
		...
		// Legacy Hook 'cache_ordered_forums' Removed //
		...
		if ($userid)
		{
			$query = "
				SELECT subscribeforumid, $counter_select, user.usergroupid, user.homepage, user.options AS useroptions, IF(userlist.friend = 'yes', 1, 0) AS isfriend
					". iif($options['threadmarking'], ', forumread.readtime AS forumread') . "
				...
				" . iif($options['threadmarking'], " LEFT JOIN " . TABLE_PREFIX . "forumread AS forumread ON (forumread.forumid = forum.forumid AND forumread.userid = $userid)") . "
		...
		else
		{
			$query = "
				SELECT $counter_select, user.usergroupid, user.homepage, user.options AS useroptions, IF(userlist.friend = 'yes', 1, 0) AS isfriend
					". iif($options['threadmarking'] AND $vbulletin->userinfo['userid'], ', forumread.readtime AS forumread') . "
				FROM " . TABLE_PREFIX . "forum AS forum
				" . iif($options['threadmarking'] AND $vbulletin->userinfo['userid'], " LEFT JOIN " . TABLE_PREFIX . "forumread AS forumread ON (forumread.forumid = forum.forumid AND forumread.userid = " .  $vbulletin->userinfo['userid'] . ")") . "
				LEFT JOIN " . TABLE_PREFIX . "user AS user ON (user.userid = forum.lastposterid)
				LEFT JOIN " . TABLE_PREFIX . "userlist AS userlist ON (userlist.relationid = user.userid AND userlist.type = 'buddy' AND userlist.userid = " . $vbulletin->userinfo['userid'] . ")
				$tachyjoin
			";
...
function construct_quick_nav($navpopup = array(), $parentid = -1, $addbox = true, $override = false)
{
	...
	if (empty($navpopup['id']))
	{
		$navpopup['id'] = 'navpopup';
	}

	//This ensures this code won't be run for vb5, hwhild
	if ($complete OR empty($vbulletin->userinfo['permissions'])
		OR !($vbulletin->userinfo['permissions']['forumpermissions'] & $vbulletin->bf_ugp_forumpermissions['canview']))
	{
		return;
	}
	...
	foreach($vbulletin->iforumcache["$parentid"] AS $forumid)
	{
		...
		if ((!($forumperms & $vbulletin->bf_ugp_forumpermissions['canview']) AND ($vbulletin->forumcache["$forumid"]['showprivate'] == 1 OR (!$vbulletin->forumcache["$forumid"]['showprivate'] AND !$options['showprivateforums']))) OR !($vbulletin->forumcache["$forumid"]['options'] & $vbulletin->bf_misc_forumoptions['showonforumjump']) OR !$vbulletin->forumcache["$forumid"]['displayorder'] OR !($vbulletin->forumcache["$forumid"]['options'] & $vbulletin->bf_misc_forumoptions['active']))
		{
			continue;
		}
		else
...
function fetch_time_data()
{
	...
	$options['hourdiff'] = (date('Z', TIMENOW) / 3600 - $vbulletin->userinfo['tzoffset']) * 3600;
	...
	$datenow       = vbdate($options['dateformat'], TIMENOW);
	$timenow       = vbdate($options['timeformat'], TIMENOW);
...
function fetch_moderator_permissions($forumid, $userid = -1, $useglobalperms = false)
{
	...
	else
	{
		...
		$perms = $vbulletin->db->query_read_slave("
			(SELECT permissions, permissions2, FIND_IN_SET(nodeid, '" . fetch_forum_parent_list($forumid) . "') AS pos, nodeid
			FROM " . TABLE_PREFIX . "moderator
			WHERE userid = $userid $forumlist
			ORDER BY pos ASC
			LIMIT 1)
			UNION
			(SELECT permissions, permissions2, 0, nodeid
			FROM " . TABLE_PREFIX . "moderator
			WHERE userid = $userid AND nodeid = -1
			)
		");
...
function can_moderate($forumid = 0, $do = '', $userid = -1, $usergroupids = '')
{
	...
	else
	{
		if (!$usergroupids)
		{
			$tempuser = vB::getDbAssertor()->getRow('vBForum:fetchusermembergroups', array(
		...
		$supermodcheck = vB::getDbAssertor()->getField('vBForum:supermodcheck', array(
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
			'usergroupids' => $usergroupids,
			'ismoderator' => $vbulletin->bf_ugp_adminpermissions['ismoderator'],
		));
		if ($supermodcheck)
		{
			DEVDEBUG('  USER IS A SUPER MODERATOR');
	...
	{ // just check to see if the user is a moderator of any forum
		...
		{ // imodcache is not set - do a query
			...
			$ismod_all = vB::getDbAssertor()->assertQuery('vBForum:isModAll', array(
	...
	{ // check to see if user is a moderator of specific forum
		...
		{ // check if user is a mod and has permissions to '$do'
			...
			else
			{
				...
				if (!isset($permission))
				{
					// Legacy Hook 'can_moderate_forum' Removed //
				}
				return $return;
...
function fetch_stylevars(&$style, $userinfo)
{
	...
	if ($userinfo['lang_options'] & $bitfields['direction'])
	{
	...
	if ($userinfo['lang_options'] & $bitfields['dirmark'])
	{
		vB_Template_Runtime::addStyleVar('dirmark', ($userinfo['lang_options'] & $bitfields['direction']) ? '&lrm;' : '&rlm;');
	}
...
// 		/*insert query*/
// 		/*
// 		vB::getDbAssertor()->assertQuery('vBForum:insertdatastore', array(
// 			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
// 			'title' => trim($title),
// 			'data' => trim($data),
// 			'unserialize' => intval($unserialize),
// 		));
// 		*/
// 		if (method_exists($vbulletin->datastore, 'build'))
// 		{
// 			$vbulletin->datastore->build($title, $data, $unserialize);
...
function print_output($vartext, $sendheader = true)
{
	...
	if (!VB_API AND (!empty($vbulletin->db->explain) OR $vb5_config['Misc']['debug']))
	{
		$totaltime = microtime(true) - TIMESTART;
