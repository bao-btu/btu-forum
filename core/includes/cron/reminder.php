...
foreach ($events as $event)
{
	...
	if ($update)
	{
		...
		$sql[$event[subscribeeventid]] = $dateline_from;
...
if (!empty($updateids))
{
	vB::getDbAssertor()->assertQuery('updateSubscribeEvent' , array('conditions' => $sql, 'subscribeeventid' => $updateids));
}
...
foreach ($eventlist AS $userid => $event)
{
	...
	foreach($event AS $eventid => $hour)
	{
		...
		$maildata = vB_Api::instanceInternal('phrase')
			->fetchEmailPhrases('reminderbit', array($eventinfo['title'], $hour, $vbulletin->options['bburl'], $eventinfo['eventid'], $eventinfo['calendar']), array(), $userinfo["$userid"]['languageid']);
	...
	$maildata = vB_Api::instanceInternal('phrase')
		->fetchEmailPhrases('reminder', array($username, $vbulletin->options['bbtitle'], $reminderbits, $vbulletin->options['bburl']), array($vbulletin->options['bbtitle']), $userinfo["$userid"]['languageid']);
	vB_Mail::vbmail($userinfo["$userid"]['email'], $maildata['subject'], $maildata['message']);

	if ($vb5_config['Misc']['debug'] AND VB_AREA == 'AdminCP')
	{
		"<pre>"; echo $maildata['subject']; echo "</pre>"; echo "<pre>"; echo $maildata['message']; echo "</pre><br />";
	}
}
