...
class vB_Collection_Factory
{
	...
	function &create($collection_type, $parent_id = false, $page = 1, $quantity = false, $descending = true, $no_limit = false)
	{
		...
		// Legacy Hook 'collection_factory_create' Removed //
...
class vB_Legacy_Collection
{
	...
	function fetch()
	{
		...
		do
		{
			...
			// get total
			list($this->total) = $this->registry->db->query_first("SELECT FOUND_ROWS()", vB_Database::DBARRAY_NUM);
...
class vB_Group_Collection_Message extends vB_Group_Collection
{
	...
	function collection_sql($force_refresh = false)
	{
		...
		/* Legacy Hook $this->view_query_hook Removed */
		...
		$sql .= "	FROM " . TABLE_PREFIX . "groupmessage AS gm
...
class vB_Group_Collection_Discussion extends vB_Group_Collection_Message
{
	...
	function collection_sql($force_refresh = false)
	{
		...
		$sql .= "	FROM " . TABLE_PREFIX . "discussion AS d
...
class vB_Bit_Factory
{
	...
	*/
	function create_instance($item)
	{
		...
		// Legacy Hook 'bit_factory_create' Removed //
...
class vB_Bit
{
	...
	*/
	function construct()
	{
		/* Legacy Hook $this->hook_display_start Removed */
		...
		/* Legacy Hook $this->hook_display_complete Removed */
...
class vB_Bit_Album extends vB_Bit
{
	...
	*/
	...
	function process_display()
	{
		...
		$this->item['coverthumburl'] = ($this->item['attachmentid'] ? 'attachment.php?' . vB::getCurrentSession()->get('sessionurl') . "albumid={$this->item['albumid']}&attachmentid={$this->item['attachmentid']}&thumb=1&d={$this->item['thumbnail_dateline']}" : '');
