...
class vB_SiteMapRunner
{
	...
	*/
	protected function check_environment_internal()
	{
		...
		$path = vB5_Route_Admincp::resolvePath($sitemap_path);
...
abstract class vB_SiteMap
{
	...
	*/
	final function create_sitemap_index()
	{
		...
		if ($fp = @fopen($sitemap_path . '/' . $output_filename, 'w'))
		{
			fwrite($fp, $content);
			fclose($fp);
			return true;
		...
		{
			$this->errors[] = 'Error writing : ' . $sitemap_path . '/' . $output_filename;
			return false;
		}
	}
	...
	*/
	final function create_sitemap($filename)
	{
		...
		$sitemap_path = vB5_Route_Admincp::resolvePath(vB::getDatastore()->getOption('sitemap_path'));
		if ($fp = @fopen($sitemap_path . '/' . $new_file, 'w'))
		{
			fwrite($fp, $content);
			fclose($fp);
			return $new_file;
		...
		{
			$this->errors[] = 'Error writing : ' . $sitemap_path . '/' . $new_file;
			return false;
		}
	}
	...
	*/
	public static function get_allowed_forums($check_thread_view = true)
	{
		...
		$nodes = array_keys($channels);
		$usercontext = new vB_UserContext(0, vB::getDBAssertor(), vB::getDatastore(), vB::getConfig());
		$access = $usercontext->getAllChannelAccess();
		foreach ($nodes AS $node)
		{
			if (in_array($node, $access['canview']))
			{
				$viewable_forums[] = $node;
			}
		}
	...
	*/
	...
	{
		$forum_priorities = vB::getDbAssertor()->getRows('contentpriority', array('contenttypeid' => 'forum'));

		foreach ($forum_priorities AS $f_pri)
		{
...
class vB_SiteMap_Forum extends vB_SiteMap
{
	...
	*/
	public function generate_sitemap($startat = 0, $perpage = 30000)
	{
		...
		$forums = vB::getDbAssertor()->getRows('getSiteForums', array(
			'viewable_forums' => $viewable_forums, 'startat' => intval($startat), 'perpage' => intval($perpage + 1)
		));
		...
		foreach ($forums AS $forum)
		{
			...
			$this->pagecount++;
			$this->lastid = $forum['nodeid'];

			$this->content .= $this->url_block(
				vB5_Route::buildUrl("{$forum['routeid']}|fullurl|nosession", array('nodeid' => $forum['nodeid'])),
				$forum['lastpost'],
				$this->get_effective_forum_priority($forum['nodeid'])
			);
		}
...
class vB_SiteMap_Thread extends vB_SiteMap
{
	...
	*/
	public function generate_sitemap($startat = 0, $perpage = 30000)
	{
		...
		$threads = vB::getDbAssertor()->getRows('getSiteThreads', array(
			'parents' => $viewable_forums, vB_dB_Query::PARAM_LIMIT => intval($perpage + 1),
			'startat' => intval($startat), 'exclude_ids' => $coventry
		));
		...
		foreach ($threads AS $thread)
		{
			...
			$this->pagecount++;
			$this->lastid = $thread['nodeid'];

			$this->content .= $this->url_block(
				vB5_Route::buildUrl("{$thread['routeid']}|fullurl|nosession", array('nodeid' => $thread['nodeid'])),
				$thread['lastcontent'],
				$this->get_effective_forum_priority($thread['nodeid'])
			);
		}
...
class vB_SiteMap_Cms extends vB_SiteMap
{
	...
	public function generate_sitemap($startat = 0, $perpage = 30000)
	{
		...
		while($node = next($this->nodes))
		{
			...
			$pageurl = str_replace( '/' . $vb5_config['Misc']['admincpdir'] . '/' , '/' , $rawurl);
...
class vB_SiteMap_Blog extends vB_SiteMap
{
	...
	public function generate_sitemap($startat = 0, $perpage = 30000)
	{
		...
		while($blog = $this->registry->db->fetch_array($rst))
		{
			...
			$postmonths = (intval($blog['lastcomment'])) ?
				floor((vB::getRequest()->getTimeNow() - $blog['lastcomment'])/(30*86400)) : 0;
			$agemonths = (intval($blog['dateline'])) ?
				floor((vB::getRequest()->getTimeNow() - $blog['dateline'])/(30*86400)) : 0;
	...
	public function save_data()
	{
		$this->registry->input->clean_array_gpc('p', array(
			'default' => vB_Cleaner::TYPE_NUM,
			'newauthor' => vB_Cleaner::TYPE_STR,
			'newauthor_pts' => vB_Cleaner::TYPE_NUM,
			'age_pts' => vB_Cleaner::TYPE_NUM,
			'age_max' => vB_Cleaner::TYPE_NUM,
			'c_age_pts' => vB_Cleaner::TYPE_NUM,
			'c_age_max' => vB_Cleaner::TYPE_NUM,
			'comm_pts' => vB_Cleaner::TYPE_NUM,
			'comm_max' => vB_Cleaner::TYPE_NUM,
		));
			...
			AND ($this->registry->GPC_exists['newauthor_pts']))
		{
			...
			{
				print_stop_message2(array('could_not_find_user_x',  $this->registry->GPC['newauthor']));
				return false;
			}
		}
		return true;
