...
function fetch_dot_threads_array($ids)
{
	...
	// Legacy Hook 'dot_threads_array' Removed //
...
function process_thread_array($thread, $lastread = -1, $allowicons = -1)
{
	...
	else
	{
		...
		if (can_moderate($thread['forumid']))
		{
			if ($thread['expires'])
			{
				...
				{
					$threadman =& datamanager_init('Thread', $vbulletin, vB_DataManager_Constants::ERRTYPE_STANDARD, 'threadpost');
	...
	// Legacy Hook 'threadbit_process' Removed //
