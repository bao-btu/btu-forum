...
function build_language_datastore()
{
	...
	$languages = vB::getDbAssertor()->assertQuery('language',
		array(
			...
			vB_Db_Query::COLUMNS_KEY => array('languageid', 'title', 'userselect')
		),
		'title'
	);
...
function build_language($languageid = -1, $phrasearray = 0, $reset_static=true)
{
	...
	if ($languageid == -1)
	{
		...
		if ($languages AND $languages->valid());
		{
			foreach ($languages as $language)
			{
	...
	if (empty($masterlang))
	{
		...
		$phrases = vB::getDbAssertor()->assertQuery('vBForum:fetchphrasesbyfieldname', array(
	...
	$phrases = vB::getDbAssertor()->assertQuery('vBForum:fetchphrasesbylanguageandfield', array(
		vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
		'languageid' => $languageid,
		'gettypes' => $gettypes,
	));
...
function get_language_export_xml($languageid, $product, $custom, $just_phrases, $charset = 'ISO-8859-1')
{
	...
	if ($vb5_config['Misc']['debug'] AND !empty($vblangcode) AND !empty($revision))
	{
		$languagegroup += array(
...
{
	global $vbulletin, $typeoptions, $vbphrase, $vb5_config;
...
function print_phrase_row($phrase, $editrows, $key = 0, $dir = 'ltr')
{
	...
	print_label_row(
		...
		</span>" . iif($phrase['found'], " <dfn><br /><label for=\"rvt$phrase[phraseid]\"><input type=\"checkbox\" name=\"rvt[$phrase[varname]]\" id=\"rvt$phrase[phraseid]\" value=\"$phrase[phraseid]\" tabindex=\"1\" />$vbphrase[revert_gcpglobal]</label></dfn>"),
