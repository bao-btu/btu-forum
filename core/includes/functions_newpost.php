...
function fetch_keywords_list($threadinfo, $pagetext = '')
{
	...
	if (!empty($pagetext))
	{
		...
		$badwords = vB_Api_Search::get_all_bad_words();
		$goodwords = vB_Api_Search::get_good_words();

		foreach ($sorted_counts AS $word => $count)
		{
			$word = trim($word);

			if (in_array(vB_String::vBStrToLower($word), $badwords))
			{
				continue;
			}

			if (vB_String::vbStrlen($word) <= vB::getDatastore()->getOption('minsearchlength') AND !in_array(vB_String::vBStrToLower($word), $goodwords))
			{
				continue;
			}
