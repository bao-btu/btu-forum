...
*
* @package 		vBulletin
* @version		$Revision: 74988 $
* @date 		$Date: 2013-05-13 12:14:35 -0700 (Mon, 13 May 2013) $
*
*/
...
//					$dobbcode = $forum['allowbbcode'];
		...
		if (($has_img_code & BBCODE_HAS_ATTACH) AND preg_match_all('#\[attach(?:=(right|left|config))?\](\d+)\[/attach\]#i', $fulltext, $matches))
		{
			...
			foreach($matches[2] AS $key => $attachmentid)
			{
					...
					switch($attachment['extension'])
					{
						...
						case 'pdf':
							...
							if (($settings OR ($this->registry->options['viewattachedimages'] == 1 AND $attachment['hasthumbnail'])) AND $this->registry->userinfo['showimages'])
							{
								...
								$hrefbits = array(
									'href'   => "{$this->registry->options['bburl']}/attachment.php?{$sessionurl}attachmentid=\\1&amp;d=$attachment[dateline]",
									'id'     => 'attachment\\1',
								);
								if ($lightbox)
								{
								...
								$imgbits = array(
									'src'    => "{$this->registry->options['bburl']}/attachment.php?{$sessionurl}attachmentid=\\1&amp;d=$attachment[thumbnail_dateline]",
									'border' => '0',
							...
							{	// Display the attachment with no link to bigger image
								$replace[] = ($fullsize ? '<div class="size_fullsize">' : '') .
									"<img src=\"{$this->registry->options['bburl']}/attachment.php?{$sessionurl}attachmentid=\\1&amp;d=$attachment[dateline]\" border=\"0\" alt=\""
							...
							{	// Display a link
								$replace[] = ($fullsize ? '<div class="size_fullsize">' : '') .
									"<a href=\"{$this->registry->options['bburl']}/attachment.php?{$sessionurl}attachmentid=\\1&amp;d=$attachment[dateline]\" $addtarget title=\""
								. construct_phrase($vbphrase['image_x_y_z'], $attachment['filename'], $attachment['counter'], $attachment['filesize'])
				...
				{	// Belongs to another post so we know nothing about it ... or we are not displying images so always show a link
					$replace[] = "<a href=\"{$this->registry->options['bburl']}/attachment.php?{$sessionurl}attachmentid=\\1\" \">$vbphrase[attachment] \\1</a>";
				}
	...
	*/
	function handle_bbcode_sigpic($description)
	{
		...
		{
			$sigpic_url = 'image.php?' . vB::getCurrentSession()->get('sessionurl') . 'u=' . $this->parse_userinfo['userid'] . "&amp;type=sigpic&amp;dateline=" . $this->parse_userinfo['sigpicdateline'];
		}
