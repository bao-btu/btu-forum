...
function fetch_datearray_from_timestamp($timestamp = TIMENOW)
{
	...
	if ($timestamp = $vbulletin->cleaner->clean($timestamp, vB_Cleaner::TYPE_UNIXTIME))
	{
		$datearray = array(
...
function mark_forums_read($forumid = false)
{
	...
	$return_url = vB5_Route::buildUrl($forumHome['routeid'] . '|nosession|fullurl');
	...
	else
	{
		...
		$vbulletin->input->clean_gpc('c', COOKIE_PREFIX . 'forum_view', vB_Cleaner::TYPE_STR);
