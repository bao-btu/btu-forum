...
function print_cp_header($title = '', $onload = '', $headinsert = '', $marginwidth = 0, $bodyattributes = '')
{
	...
	<script type=\"text/javascript\" src=\"{$_SERVER['x-vb-presentation-base']}/js/jquery/jquery-1.7.2.min.js\"></script>\n\r";
	echo "</head>\r\n";
...
function print_delete_confirmation($table, $itemid, $phpscript, $do, $itemname = '', $hiddenfields = 0, $extra = '', $titlename = 'title', $idfield = '')
{
	...
	{
		case 'infraction':
			$item = $assertor->getRow('vBForum:infraction', array('nodeid' => $itemid));
			$item['title'] = (!empty($item) AND isset($item['nodeid'])) ? $item['nodeid'] : '';
			break;
		case 'reputation':
