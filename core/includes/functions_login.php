...
//			{
//				$ip = IPADDRESS;
//				$maildata = vB_Api::instanceInternal('phrase')->fetchEmailPhrases('accountlocked', array($user['username'], $vbulletin->option['bbtitle'], $ip), array($vbulletin->option['bbtitle']), $user['languageid']);
...
//		vbsetcookie('password', md5($vbulletin->userinfo['password'] . vB_Request_Web::COOKIE_SALT), true, true, true);
//	}
...
//		$username
//	);
//	if ($userinfo = $vbulletin->db->query_first("SELECT userid, usergroupid, membergroupids, infractiongroupids, username, password, salt FROM " . TABLE_PREFIX . "user WHERE username = '" . $vbulletin->db->escape_string(htmlspecialchars_uni($username)) . "'"))
//	{
//		if (
//			$userinfo['password'] != iif($password AND !$md5password, md5(md5($password) . $userinfo['salt']), '') AND
//			$userinfo['password'] != iif($md5password, md5($md5password . $userinfo['salt']), '') AND
//			$userinfo['password'] != iif($md5password_utf, md5($md5password_utf . $userinfo['salt']), '')
//		)
//		{
//			$return_value = false;
//			// Legacy Hook 'login_verify_failure_password' Removed //
...
//		}
//
//		// authentication is valid, so create new session (this modifies vbulletin->userinfo)
//		vB::setCurrentSession(vB_Session_Web::getSession($userinfo['userid']));
//
//		if ($send_cookies)
//		{
...
//	vB::setCurrentSession(vB_Session_Web::getSession($userId));
//	$return_value = true;
//	// Legacy Hook 'login_verify_success' Removed //
//	return $return_value;
//}
...
//
//	if ($vbulletin->session->created == true AND vB::getCurrentSession()->get('userid') == 0)
//	{
...
//	else
//	{
////		$newsession = new vB_Session($vbulletin, '', $vbulletin->userinfo['userid'], '', vB::getCurrentSession()->get('styleid'), vB::getCurrentSession()->get('languageid'));
//		$newsession = vB_Session::getNewSession(vB::getDbAssertor(), vB::getDatastore(), vB::getConfig(), '', $vbulletin->userinfo['userid'], '', vB::getCurrentSession()->get('styleid'), vB::getCurrentSession()->get('languageid'));
//	}
...
//				vbsetcookie('password', md5($vbulletin->userinfo['password'] . vB_Request_Web::COOKIE_SALT), false, true, true);
//			}
//		}
//	}
//
...
//	}
//
//	// Legacy Hook 'login_process' Removed //
//}
	...
	if (!VB_API)
	{
		...
		if ($globalgroup)
		{
			...
			if ($vbulletin->GPC['postvars'])
			{
				...
				{
					$vbulletin->userinfo['securitytoken_raw'] = sha1($vbulletin->userinfo['userid'] . sha1($vbulletin->userinfo['salt']) . sha1(vB_Request_Web::COOKIE_SALT));
