...
function do_facebook_redirect()
{
	...
	if (!isset($dofbredirect))
	{
		...
		$vbulletin->input->clean_array_gpc('r', array(
			'dofbredirect'	=> vB_Cleaner::TYPE_BOOL
		));
...
function get_fbcanonicalurl()
{
	...
	if (empty($fbcanonicalurl))
	{
		...
		{
			global $threadinfo;
			$fbcanonicalurl = create_full_url(fetch_seo_url('thread|js', $threadinfo, null, null, null, true));
		}
		else if (THIS_SCRIPT == 'entry')
		{
			global $bloginfo;
			$fbcanonicalurl = create_full_url(fetch_seo_url('entry|js', $bloginfo, null, null, null, true));
		}
...
function get_fbprofilepicurl()
{
	...
	if (empty($picurl))
	{
		$picurl = !empty($vbulletin->userinfo['fbuserid']) ? (Facebook::$DOMAIN_MAP['graph'].$vbulletin->userinfo['fbuserid'].'/picture') : htmlspecialchars_uni($vbulletin->input->clean_gpc('c', COOKIE_PREFIX . 'fbprofilepicurl', vB_Cleaner::TYPE_STR));
...
*/
function save_fbdata($userdata, $is_registering = true)
{
	global $vbulletin;

	// save the data from the import form
	save_fbimportform_into_userdm($userdata, $is_registering);
...
function check_fbpublishcheckbox()
{
	...
	$vbulletin->input->clean_array_gpc('p', array(
		'fb_dopublish' => vB_Cleaner::TYPE_BOOL
	));
...
* @param	bool	True if the user is registering, false if the user is already registered (linking accounts)
*/
function save_fbimportform_into_userdm($userdata, $is_registering = true)
{
	global $vbulletin;
...
function save_fbavatar($userdata, $avatarurl = '', $do_fallback = true, $fallback_avatarurl = '')
{
	...
	$upload->data = new vB_DataManager_Userpic_Avatar($vbulletin, vB_DataManager_Constants::ERRTYPE_STANDARD);
