		...
		$weekrange = '', $thismonth_only = false)
{
	...
	while (++$rows <= 6)
	{
		...
		foreach ($userweek AS $daycount => $curday)
		{
			...
			else
			{
				...
				if (!empty($eastercache["$month-$day-$year"]))
				{
					...
					if ($fullcalendar)
					{
						...
						// Legacy Hook 'calendar_displaymonth_event' Removed //
			...
			// Legacy Hook 'calendar_displaymonth_daybit' Removed //
		...
		// Legacy Hook 'calendar_displaymonth_weekbit' Removed //
	...
	// Legacy Hook 'calendar_displaymonth_month' Removed //
	...
	$events = $vbulletin->db->query_read_slave("
		...
		" . ($vbulletin->options['avatarenabled'] ?
			",avatar.avatarpath, NOT ISNULL(customavatar.userid) AS hascustomavatar, customavatar.dateline AS avatardateline,
				customavatar.width AS avwidth,customavatar.height AS avheight, customavatar.width_thumb AS avwidth_thumb,
