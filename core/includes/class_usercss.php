...
* @date		$Date: 2011-08-18 13:25:24 -0700 (Thu, 18 Aug 2011) $
*/
class vB_UserCSS
{
 	/**
	...
	*/
	...
	/**
	 * The vBulletin database assertor object
	 *
	 * @var vB_dB_Assertor
	 */
	...
	*/
	function vB_UserCSS(&$registry, $userid, $fetch_data = true)
	{
		...
		$this->assertor = vB5_vB::get_db_assertor();
	...
	*/
	...
	{
		$userinfo = vB5_vB::getCurrentSession()->fetch_userinfo();
		...
		else if ($user = $this->assertor->getRow('user', array('userid' => $userid)))
		{
			$this->userid = $userid;
		...
		{
			global $vbphrase;
			$this->error[] = fetch_error('invalidid', $vbphrase['user'], $options['contactuslink']);
			return false;
		}
		...
		if (!empty($this->store))
		{
			...
			foreach ($this->store as $selector => $properties)
			{
				foreach ($properties as $property => $value)
				{
					...
					$this->assertor->delete('usercss', array(
							'userid' => $this->userid,
							'selector' => $selector,
							'property' => $property
					));
	...
	*/
	function update_css_cache()
	{
		...
		$this->assertor->assertQuery('replaceUserCssCache', array(
				'usercss' => $this->userid,
				'cachedcss' => $buildcss,
				'buildpermissions' => intval($this->permissions['usercsspermissions'])
		));
	...
	*/
	function check_css_permission($selector, $property, $permissions)
	{
		...
		{
			return ($options['socnet'] & $bf_misc_socnet['enable_albums'] AND $this->permissions['usercsspermissions'] & $bf_ugp_usercsspermissions["$permfield"]) ? true : false;
		}

		return ($this->permissions['usercsspermissions'] & $bf_ugp_usercsspermissions["$permfield"]) ? true : false;
	}
	...
	*/
	function build_display_array()
	{
		...
		// hook usercss_build_display_array
	...
	*/
	function verify_image(&$value)
	{
		...
		if ($foundalbum AND $foundpicture AND $picture = $this->assertor->getRow('getUserPictures', array(
				'attachmentid' => intval($attachmentid[1]),
				'contenttypeid' => $contenttypeid,
				'state' => array('profile', 'public'),
				'userid' => $this->userid,
				'albumid' => intval($albumid[1])
		)))
	 	{
