...
function construct_reminders_summary_string($date, $showevents = 1)
{
	...
	if (is_array($event["$date"]))
	{
		if ($showevents)
		{
			...
			{
				$out .= "<li" . iif(!$reminder['completedby'], ' style="font-weight:bold"', '') . "><a href=\"reminder.php?" . vB::getCurrentSession()->get('sessionurl') . "do=view&amp;reminderid=$reminder[reminderid]\">$reminder[title]</a></li>\n";
			}
			$out .= "</span>\n";
		}
		else
	...
	while ($monthcomplete == 0 AND $safety < 50)
	{
		...
		for ($i=0; $i<7; $i++)
		{
			...
			if ($monthstarted AND !$monthcomplete)
			{
				...
				{
					echo "\t<td class=\"alt2\"><span class=\"smallfont\">" . iif(construct_reminders_summary_string("$month-$curday", 0), "<a href=\"reminder.php?" . vB::getCurrentSession()->get('sessionurl') . "do=viewday&day[$year-$month-$curday]=1\"><b class=\"col-i\">$printday</b></a>", "$printday") . "</span></td>\n";
				}
				$curday++;
...
function print_reminder($event)
{
	...
	print_label_row($vbphrase['adminfunctions_reminder_showevent_postedby'], "<a href=\"../member.php?" . vB::getCurrentSession()->get('sessionurl') . "u=$event[userid]\" target=\"_blank\">$event[username]</a>");
	...
	print_table_footer(2, '
		...
		<input type="button" class="button" value="' . $vbphrase['adminfunctions_reminder_showevent_newevent'] . '" tabindex="1" onclick="'."window.location='reminder.php?" . vB::getCurrentSession()->get('sessionurl') . "do=add&amp;year=" . vbdate('Y', $event['duedate']) . "&amp;month=" . vbdate('n', $event['duedate']) . "&amp;add_day=" . vbdate('j', $event['duedate']) . "';" . '" />
	');
...
function print_confirm_reminder_completed()
{
	...
	function js_confirm_completion(id)
	{
		...
		{
			window.location = 'reminder.php?<?php echo vB::getCurrentSession()->get('sessionurl'); ?>do=complete&reminderid=' + id;
		}
		else
		{
			return false;
...
// prints out all reminders for the appropriate control panel
	...
	if ($vbulletin->db->num_rows($reminders))
	{
		...
		while ($reminder = $vbulletin->db->fetch_array($reminders))
		{
			...
			$cell[] = '<p class="smallfont">' . construct_link_code($vbphrase['edit'], "reminder.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;id[$reminder[adminreminderid]]=1") . '</p>';
...
function fetch_reminders_array()
{
	...
	if ($reminders['total'] != 0)
	{
		...
		{
			window.location = 'reminder.php?' . vB::getCurrentSession()->get('sessionurl_js');
		}
		// -->
		</script>\n";
	}
