...
//		);
	...
	*
	*/
	function step_9() // DUPLICATE OF STEP 5 //
	{
		$this->skip_message();
	}
...
//		);
	...
	*/
	function step_58()
	{
		if (!empty($this->registry->bf_misc_useroptions) AND !empty($this->registry->bf_misc_useroptions['vbasset_enable']))
		{
			$this->run_query(
		...
		if ($db_alter->fetch_table_info('albumpicture'))
		{
			...
			if ($this->db->num_rows($pictures))
			{
				...
				while ($picture = $this->db->fetch_array($pictures))
				{
					...
					$attachdm = new vB_DataManager_AttachmentFiledata(vB::get_registry(), vB_DataManager_Constants::ERRTYPE_UPGRADE);
		...
		if ($db_alter->fetch_table_info('albumpicture'))
		{
			...
			if ($this->db->num_rows($pictures))
			{
				...
				while ($picture = $this->db->fetch_array($pictures))
				{
					...
					$attachdm = new vB_DataManager_AttachmentFiledata(vB::get_registry(), vB_DataManager_Constants::ERRTYPE_CP);
