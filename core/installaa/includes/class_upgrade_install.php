	...
	*/
	function step_5($data = null)
	{
		if (!$data['response']&&vB_Upgrade::isCLI()){ //try to use config data from cli-specific file
			...
			if(!empty($vb5_config['cli']['forum_data'])) $data = array('htmlsubmit' => "yes",'response' => "yes", 'htmldata'   => $vb5_config['cli']['forum_data']);
		}
	...
	*/
	function step_6($data = null)
	{
		if (!$data['response']&&vB_Upgrade::isCLI()){
			$vb5_config = vB_Upgrade_Cli::getConfigCLI();
			if(!empty($vb5_config['cli']['cookie_data'])) $data = array('htmlsubmit' => "yes",'response' => "yes", 'htmldata'   => $vb5_config['cli']['cookie_data']);
		}
		if ($data['response'])
		{
		...
		$script = substr(SCRIPT, 0, strrpos(SCRIPT, '/')); // remove 'core' directory
		$cookiepaths = construct_select_options(fetch_valid_cookiepaths($script), '/');
...
<tr valign="top">
	...
	*/
	function step_7($data = null)
	{
		...
		if (!$data['response']&&vB_Upgrade::isCLI()){
			$vb5_config = vB_Upgrade_Cli::getConfigCLI();
			if(!empty($vb5_config['cli']['user_data'])) $data = array('htmlsubmit' => "yes",'response' => "yes", 'htmldata'   => $vb5_config['cli']['user_data']);
		}
		if ($data['response'])
		{
			...
			if (empty($errors))
			{
				...
				require_once(DIR . '/vb/datamanager/user.php');

				if (vB_Upgrade::isCLI()&&$vb5_config['cli']['use_default_admin_account']){
					foreach ($vb5_config['cli']['saas_admin']['initial_user_profile']['user_options'] AS $bitfield)
						{
					...
					$this->db->query_write("
						INSERT INTO " . TABLE_PREFIX . "user
							(userid, username, salt, password, email, usertitle, joindate, lastvisit, lastactivity, usergroupid, passworddate, options, showvbcode, membergroupids)
						VALUES (
							2,
							...
							" . TIMENOW . ",
							" . TIMENOW . ",
							{$vb5_config['cli']['saas_admin']['initial_user_profile']['group_id']},
							FROM_UNIXTIME(" . TIMENOW . "),
							$saas_admin_useroption,
							2,
							'{$vb5_config['cli']['saas_admin']['initial_user_profile']['member_groups_ids']}'
						)
					");
					...
					VALUES
						($userid, " . (array_sum($vb5_config['cli']['saas_admin']['initial_admin_profile'])-3) . ")
					");
					/*insert query*/
				...
				$salt = fetch_user_salt();
					/*insert query, super-admin*/
					$this->db->query_write("
						INSERT INTO " . TABLE_PREFIX . "user
							(userid,username, salt, password, email, usertitle, joindate, lastvisit, lastactivity, usergroupid, passworddate, options, showvbcode)
						VALUES (
							1,
							'" . $this->db->escape_string(htmlspecialchars_uni($vb5_config['cli']['superadmin']['admin_data']['username'])) . "',
							'" . $this->db->escape_string($salt) . "',
							'" . $this->db->escape_string(md5(md5($vb5_config['cli']['superadmin']['admin_data']['password']) . $salt)) . "',
							'" . $this->db->escape_string($vb5_config['cli']['superadmin']['admin_data']['email']) . "',
							'" . $this->db->escape_string($this->phrase['install']['usergroup_admin_usertitle']) . "',
							" . TIMENOW . ",
							" . TIMENOW . ",
							" . TIMENOW . ",
