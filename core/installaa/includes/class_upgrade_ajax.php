...
class vB_Upgrade_Ajax extends vB_Upgrade_Abstract
{
	...
	*/
	protected function init($script)
	{
		...
		else
		{
			...
			if ($proceed)
			{
				if (!empty($_REQUEST['version']) AND $this->registry->GPC['version'] AND $this->versions[$this->registry->GPC['version']])
				{
					$this->scriptinfo = array(
	...
	* @var	Object	vB_XML_Builder_Ajax
	* @var	bool		Check table status
	* @var	string	Response from a prompt
	* @var	bool		Received an HTML form response
	* @var	array		Values returned from an HTML form
	...
	*/
	private function print_html()
	{
		...
		<head>
			...
			<title><?php echo $this->htmloptions['setuptype'] . " " . $steptitle; ?></title>
			...
			* html .floatcontainer {
			...
			<!--
				...
				<?php $baseurl_core = class_exists('vB5_Config') ? vB5_Config::instance()->baseurl_core : '..'; ?>
				var CLEARGIFURL = "<?php echo $baseurl_core . '/' . $this->registry->options['cleargifurl']; ?>";
		...
		<div id="acp-logo-bar" class="navbody floatcontainer">
			...
			<div class="logo">
				<img src="../cpstyles/<?php echo $this->registry->options['cpstylefolder']; ?>/cp_logo.png" alt="" title="vBulletin 5 &copy; <?php echo date('Y'); ?> vBulletin Solutions, Inc. All rights reserved." />
			</div>
			<div class="notice">
		...
		<div id="all">
			...
			<div class="tborder<?php if (!$this->htmloptions['mismatch']) { echo " hidden"; } ?>" id="mismatch">
				...
				<div class="messagebody logincontrols">
					<?php
						if ($this->htmloptions['version'] == 'final')
						{
							end($this->versions);
							$this->htmloptions['version'] = key($this->versions);
						}
						echo construct_phrase($this->phrase['core']['wrong_version'], $this->versions[$this->htmloptions['version']], $this->registry->options['templateversion']);
					?>
