...
class vB_Upgrade_final extends vB_Upgrade_Version
{
	...
	*/
	...
	function step_5()
	{
		...
		$blogParentId = vB_Library::instance('Blog')->getBlogChannel();
		...
		while ($channel = array_shift($queue))
		{
			...
			REPLACE INTO `" . TABLE_PREFIX . "routenew`
				...
				VALUES
				('$newprefix', '$newprefix(?:/page(?P<pagenum>[0-9]+))?', 'vB5_Route_Channel', 'page', 'index', '', '" . serialize(array('channelid'=>$channel['nodeid'], 'pagenum'=>'$pagenum', 'pageid'=>$pageid)) . "', {$channel['nodeid']}, '" . vB_Xml_Export_Route::createGUID(array()) . "');
				"
			);
				...
				while (($file = readdir($dh)) !== false)
				{
					if (strpos($file, 'vbulletin-custom-language-') === 0)
					{
						...
						else
						{
							...
							$check = xml_import_language($xml, 0, '', false, true, !defined('SUPPRESS_KEEPALIVE_ECHO'));
							if ($check === false)
							{
								$this->skip_message();
							}
	...
	*/
	...
	/**
	 * Step #13 - Rebuild filesystem template cache
	 */
	function step_13()
	{
