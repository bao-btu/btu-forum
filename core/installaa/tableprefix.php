...
// Fetch the configuration for the admicncp directory 
$vb5_config = vB::getConfig();

require_once(DIR . "/{$vb5_config['Misc']['admincpdir']}/global.php");
...
if ($_POST['do'] == 'rename')
{
	$vbulletin->input->clean_array_gpc('p', array(
		'newprefix' => vB_Cleaner::TYPE_STR,
		'rename' => vB_Cleaner::TYPE_ARRAY,
	));
...
if ($_POST['do'] == 'confirm')
{
	...
	{
		print_stop_message2('sorry_no_tables_were_submitted_to_be_renamed');
	}
	else
	{
		if ($vbulletin->GPC['newprefix'] == TABLE_PREFIX)
...
if ($_REQUEST['do'] == 'choose')
{
	...
	print_label_row($vbphrase['table_name_gmaintenance'], '<input type="button" class="button" value=" ' .  $vbphrase['set_all_yes']  . ' " onclick="js_check_all_option(this.form, 1);" /> <input type="button" class="button" value="  ' . $vbphrase['set_all_no']  . ' " onclick="js_check_all_option(this.form, 0);" />', 'tfoot');
