	...
	* **/
	...
	(
		'getParents' => array(vB_dB_Query::QUERYTYPE_KEY => vB_dB_Query::QUERY_SELECT,
			'query_string' =>  "SELECT cl.*, node.nodeid, node.userid, node.parentid, node.routeid, node.title, node.urlident, node.contenttypeid, node.publishdate, node.unpublishdate, node.showpublished, node.starter
		...
		'incrementUserPostCount' => array(
			...
			'query_string' => "
				...
				SET posts = posts + 1, lastpost = {timenow}, lastpostid = {lastnodeid}
				WHERE userid = {userid}
			",
		),
		...
		*/
		...
		'getParentLastRead' => array(
			vB_dB_Query::QUERYTYPE_KEY => vB_dB_Query::QUERY_SELECT,
			'query_string' => 'SELECT cl.parent, nr.readtime, max(child.lastcontent) AS lastcontent FROM {TABLE_PREFIX}closure AS cl
			INNER JOIN {TABLE_PREFIX}node AS n ON n.nodeid = cl.parent
			INNER JOIN {TABLE_PREFIX}node AS child ON child.nodeid = n.parentid
			LEFT JOIN {TABLE_PREFIX}noderead AS nr ON nr.nodeid = cl.parent AND nr.userid = {userid}
			LEFT JOIN {TABLE_PREFIX}noderead AS chnr ON chnr.nodeid = child.nodeid AND nr.userid = {userid}
			WHERE cl.child = {nodeid} AND cl.depth > 0 AND (chnr.nodeid IS NULL or chnr.readtime >= child.lastcontent)
			AND n.nodeid IN ({canview})
			GROUP BY cl.parent, nr.readtime
			ORDER BY cl.depth;',
		),
		'deleteInfractionByUser' => array(
			vB_dB_Query::QUERYTYPE_KEY => vB_dB_Query::QUERY_SELECT,
			'query_string' => '
				DELETE i FROM {TABLE_PREFIX}infraction i
	...
	public function getChannel($params, $db, $check_only = false)
	{
		...
		else
		{
			$params = vB::getCleaner()->cleanArray($params, array(
				...
				vB_Api_Search::FILTER_FOLLOW 	=> vB_Cleaner::TYPE_STR,
	...
	public function listNotifications($params, $db, $check_only = false)
	{
		...
		else
		{
			$params = vB::getCleaner()->cleanArray($params, array(
				'sortDir' => vB_Cleaner::TYPE_STR,
				'folderid' => vB_Cleaner::TYPE_UINT,
				'about'      => vB_Cleaner::TYPE_NOCLEAN, // it is being cleaned in its if(!empty()) section
				vB_dB_Query::PARAM_LIMIT => vB_Cleaner::TYPE_UINT,
				vB_dB_Query::PARAM_LIMITPAGE => vB_Cleaner::TYPE_UINT,
			));
	...
	***/
	function moveNodes($params, $db, $check_only = false)
	{
		if ($check_only)
		{
			...
			$params = vB::getCleaner()->cleanArray($params, array(
				'nodeids' => vB_Cleaner::TYPE_ARRAY_UINT,
				'to_parent' => vB_Cleaner::TYPE_UINT
			));
	...
	public function cloneNodes($params, $db, $check_only = false)
	{
		...
		$params = vB::getCleaner()->cleanArray($params, array(
			'nodeids' => vB_Cleaner::TYPE_ARRAY_UINT,
			'parentid' => vB_Cleaner::TYPE_UINT,
			'newtitle' => vB_Cleaner::TYPE_STR
		));
		$params['newtitle'] = $db->escape_string($params['newtitle']);
		...
		foreach ($nodes as $node)
		{
			...
			foreach ($children as $k => $closure)
			{
				...
				$db->query_write("INSERT INTO " . TABLE_PREFIX . "closure (parent, child, depth, publishdate)
					...
					WHERE child = " . $child['nodeid'] );

				$fields = array(
					'lastcontentid' => $newnodeid,
					'totalcount' => 0,
					'textcount' => 0,
					'totalunpubcount' => 0,
					'textunpubcount' => 0
				);
				if (!empty($oldnewnodes[$child['parentid']]) AND $oldnewnodes[$child['parentid']] != $child['parentid'])
				{
					$fields['parentid'] = $oldnewnodes[$child['parentid']];
				}

				if (!empty($oldnewnodes[$child['starter']]) AND $oldnewnodes[$child['starter']] != $child['starter'])
				{
					$fields['starter'] = $oldnewnodes[$child['starter']];
				}
				if (!empty($fields))
				{
					vB::getDbAssertor()->update('vBForum:node', $fields, array('nodeid' => $newnodeid));
				}
				...
				switch ($child['contenttypeid'])
				{
					...
					case vB_Types::instance()->getContentTypeID('vBForum_Infraction'):
						// Infraction
						$this->cloneNodeRecord($db, 'text', $child['nodeid'], $newnodeid);
						$this->cloneNodeRecord($db, 'infraction', $child['nodeid'], $newnodeid);
						break;
				...
				if ($index == 0)
				{
					// Removed setting of starter. Let vBForum:moveNodes at the very end of it.
					...
					$result = $node_api->moveNodes(array($newnodeid), $params['parentid'], false, false, false);
					$starter = $newnodeid;
				}
				else
				{
					...
					$moveTo = $starter;
					if ($oldnewnodes[$child['parentid']])
					{
						$moveTo = $oldnewnodes[$child['parentid']];
					}
					$node_api->moveNodes(array($newnodeid), $moveTo);
	...
	function getModeration($params, $db, $check_only = false)
	{
		...
		{
			$params['canModerate'] = array($params['canModerate']);
		}

		$params['canModerate'] = vB::getCleaner()->clean($params['canModerate'], vB_Cleaner::TYPE_ARRAY_UINT);
		...
		if (!empty($params['userid']))
		{
			...
			$params['userid'] = vB::getCleaner()->clean($params['userid'], vB_Cleaner::TYPE_ARRAY_UINT);
	...
	function getSubscriptionUsers($params, $db, $check_only = false)
	{
		...
		$params = vB::getCleaner()->cleanArray($params, array(
			'threadids' => vB_Cleaner::TYPE_ARRAY_UINT,
			'userids' => vB_Cleaner::TYPE_ARRAY_UINT
		));
	...
	function getUserFollowing($params, $db, $check_only = false)
	{
		...
		else
		{
			...
			)
			{
				$sql[] = "
					### Users ###
					SELECT follow.username AS title, follow.userid AS keyval, 'user' AS sourcetable, IFNULL(follow.lastpost, follow.joindate) AS lastactivity, follow.posts as activity, 'Member' AS type
			...
			)
			{
				...
				$sql[] = "
					...
					follow.totalcount AS activity, 'Channel' AS type
					FROM " . TABLE_PREFIX . "node AS follow
					INNER JOIN " . TABLE_PREFIX . "groupintopic AS git ON git.nodeid = follow.nodeid AND git.userid = " . $params['userid'] . "
	 ...
	 */
	/*
	*  Removed quickUpdateBookmarkSites for VBV-7927 as it was not referenced anywhere in the actual code
	*/
	 ...
	 */
	/*
	*  Removed quickUpdateBookmarkSitesSwap for VBV-7927 as it was not referenced anywhere in the actual code
	*/
	...
	public function insertSetting($params, $db, $check_only = false)
	{
		...
		else
		{
			$params = vB::getCleaner()->cleanArray($params, array(
				...
				'displayorder' => vB_Cleaner::TYPE_UINT,
				'volatile' => vB_Cleaner::TYPE_UINT,
				'advanced' => vB_Cleaner::TYPE_UINT,
				'product' => vB_Cleaner::TYPE_STR,
				'validationcode' => vB_Cleaner::TYPE_STR,
				'blacklist' => vB_Cleaner::TYPE_UINT,
			));
...
// 		}
	...
	public function fetchPendingPosts($params, $db, $check_only)
	{
		...
		else
		{
			...
			$sql = "SELECT DISTINCT node.nodeid, node.contenttypeid
			...
			{
				$sql .= 'AND node.created > ' . $params['cutofftime'] . "\n";
			}
			// fetch not approved
			$sql .= "AND node.approved = 0 AND node.showpublished <> 0 \n";
			$sql .= "ORDER BY node.publishdate DESC LIMIT\n";
