...
class vBSphinxSearch_Core extends vB_Search_Core
{
	...
	public function __construct()
	{
		...
		$this->shpinxDB = new vB_Database();
		$this->shpinxDB->connect('', $config['Misc']['sphinx_host'], $config['Misc']['sphinx_port'], '', '', false);
	...
	public function indexText($node, $title, $text, $skip_prev_index = false)
	{
		...
		{
			$crc = sprintf('%u', crc32($title . $text));
		}
		...
		$values = array("'" . $this->shpinxDB->escape_string($title) . "'", "'" . $this->shpinxDB->escape_string($text) . "'");

		foreach ($this->attributes_definition as $key => $type)
		{
			$values[] = $this->quote_smart($key, $node[$key], false);
		}

		$this->shpinxDB->query_write($query = (empty($changes['existing']) ? 'INSERT' : 'REPLACE') . " INTO " . $this->table . "(
	...
	{
		$config = vB::getConfig();
		$command = $config['Misc']['sphinx_path'] . '/bin/indexer --config ' . $config['Misc']['sphinx_config'] . ' --sighup-each --rotate ' . ($silent ? '--quiet ' : '') . ($this->table . '_disk');
		if ($silent)
		{
		...
		$this->restart($silent);
		$this->shpinxDB->query_write($query = "TRUNCATE RTINDEX " . $this->table);
		$this->shpinxDB->query_write("ATTACH INDEX " . ($this->table . '_disk') . " TO RTINDEX " . $this->table);
	...
	public function attributeChanged($nodeid)
	{
		...
		$this->shpinxDB->query_write($query);
		$config = vB::getConfig();
		if (!empty($config['Misc']['debug_sql']) OR self::DEBUG)
		{
			echo "$query;\n";
	...
	public function delete($nodeid, $node = false)
	{
		...
		$config = vB::getConfig();
		$this->shpinxDB->query_write($query = "DELETE FROM " . $this->table . " WHERE id = " . $nodeid);
		if (!empty($config['Misc']['debug_sql']) OR self::DEBUG)
		{
			echo "$query;\n";
		}

		$this->shpinxDB->query_write($query = "DELETE FROM " . $this->table . " WHERE closure = $nodeid");
		if (!empty($config['Misc']['debug_sql']) OR self::DEBUG)
		{
			echo "$query;\n";
		}
	...
	public function getResults(vB_Search_Criteria $criteria)
	{
		...
		$cacheKey = $this->getTwoPassResults($criteria);
		if (gettype($cacheKey) == 'object')
		{
			return $cacheKey;
		}
		...
		array_unshift($this->where, "MATCH('" . $this->shpinxDB->escape_string($criteria->get_raw_keywords()) . "')");
		...
		$results = array();
		$nodeids = array();
		while($row = $this->shpinxDB->fetch_array($res))
		{
			$value = current($row);
			$nodeids[$value] = $value;
			if ($cacheKey)
		...
		return $nodeids;
// 		$criteria->reset_keyword_filter();
// 		$criteria->add_filter('nodeid', vB_Search_Core::OP_EQ, $nodeids);
// 		return $vBDBSearch_Core->getResults($criteria);
	}
	...
	private function process_filters(vB_Search_Criteria &$criteria, $filter_method, $excludeUserSpecific = false)
	{
		foreach ($this->filters[$filter_method] as $field => $value)
		{
			switch ($field)
			{
				...
				case 'null':
					$this->where[] = "false /** " . $this->shpinxDB->escape_string(str_replace(array('*', '/', '\\'), ' ', $value)). " **/";
					break;
				case 'tag':
...
// 					{
// 						$fld = $this->shpinxDB->clean_identifier($fld);
...
// 					$this->where[''] = "(" . implode(' OR ', $qbits) . ")";
				...
				default:
					...
					$dbfield = $this->shpinxDB->clean_identifier($dbfield);
	...
	protected function process_marked_filter($marked) /** two stage */
	{
		...
		{
			$this->where[] = "0 /** no user in session **/";
		}
		...
		if ($marked == vB_API_Search::FILTER_MARKED_READ)
		{
			$this->where[] = $this->make_equals_filter('nodeid', $nodeids);
		}

		if ($marked == vB_API_Search::FILTER_MARKED_UNREAD)
		{
			$this->where[] = $this->make_notequals_filter('nodeid', $nodeids);
		}
	}
	...
	protected function process_channel_filters($channelid, $depth = false, $include_starter = false, $depth_exact = false, $excludeUserSpecific = false)
	{
		...
		{
			$this->what[] = '(id = starter) as is_starter';
			$this->where[] = 'is_starter = 1';
		}
	...
	protected function process_view_filters($view)
	{
		...
		switch ($view)
		{
			...
			case vB_Api_Search::FILTER_VIEW_ACTIVITY :
				...
				$this->what[] = "id = starter as id_starter";
				$this->where[] = "id_starter = 1";
	...
	protected function process_follow_filters($value, vB_Search_Criteria &$criteria) /** multi stage */
	{
		...
		if ($type == vB_Api_Search::FILTER_FOLLOWING_BOTH)
		{
			...
			$this->make_equals_filter('nodeid', $allNodeIds);
			return;
		}
	...
	{
		$existing = $this->shpinxDB->query_first($query = "SELECT * FROM " . $this->table . " WHERE id = " . $node['nodeid']);
		$assertor = vB::getDbAssertor();
		...
		if (!empty($existing))
		{
			...
			foreach ($existing as $attribute => $value)
			{
				...
				if ($node[$attribute] != $value)
				{
					...
					//we need to update one of the string attributes. string attributes can't be updated using UPDATE, we need to use REPLACE which means re-indexing the whole node
					if ($this->attributes_definition[$attribute] == 'rt_attr_string')
					{
						$need_reindex = true;
					}
				}
	...
	private function quote_smart($field, $value, $is_handled = true)
	{
		...
		if ($this->attributes_definition[$field] == 'rt_attr_string')
		{
			...
			if (is_array($value))
			{
				...
				{
					$value[$k] = "'" . $this->shpinxDB->escape_string($v) . "'";
				}
				return $value;
			}
			else
			{
				return "'" . $this->shpinxDB->escape_string($value) . "'";
			}
		}
