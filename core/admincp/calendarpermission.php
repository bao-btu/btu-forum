...
$vbulletin->input->clean_array_gpc('r', array(
	'calendarpermissionid' 	=> vB_Cleaner::TYPE_INT,
...
if ($_REQUEST['do'] == 'edit')
{
	...
	{
		$calendar = $vbulletin->db->query_first("SELECT title FROM " . TABLE_PREFIX . "calendar WHERE calendarid = " . $vbulletin->GPC['calendarid']);
		$usergroup = $vbulletin->db->query_first("SELECT title FROM " . TABLE_PREFIX . "usergroup WHERE usergroupid = " . $vbulletin->GPC['usergroupid']);
	...
	print_description_row('
		<label for="uug_1"><input type="radio" name="useusergroup" value="1" id="uug_1" tabindex="1" onclick="this.form.reset(); this.checked=true;"' . iif(!$vbulletin->GPC['calendarpermissionid'], ' checked="checked"', '') . ' />' . $vbphrase['use_default_permissions_gcppermission'] . '</label>
		<br />
...
if ($_POST['do'] == 'doupdate')
{
	$vbulletin->input->clean_array_gpc('p', array(
		...
		'calendarpermission'   => vB_Cleaner::TYPE_ARRAY
	));
	...
	if ($vbulletin->GPC['useusergroup'])
	{
		...
		if ($vbulletin->GPC['calendarpermissionid'])
		{
			$vbulletin->db->query_write("DELETE FROM " . TABLE_PREFIX . "calendarpermission WHERE calendarpermissionid = " . $vbulletin->GPC['calendarpermissionid']);
			print_stop_message2('deleted_calendar_permissions_successfully', 'calendarpermission', array('do'=>"modify#calendar" . $vbulletin->GPC['calendarid']));
		}
		else
		{
			print_stop_message2('saved_calendar_permissions_successfully', 'calendarpermission', array('do'=>"modify#calendar" . $vbulletin->GPC['calendarid']));
		}
	}
	else
	{
		...
		if ($vbulletin->GPC['calendarid'] AND !$vbulletin->GPC['calendarpermissionid'])
		{
			...
			$calendarinfo = $vbulletin->db->query_first("SELECT title AS calendartitle FROM " . TABLE_PREFIX . "calendar WHERE calendarid=" . $vbulletin->GPC['calendarid']);
...
if ($_REQUEST['do'] == 'modify')
{
	...
	print_form_header('', '');
	print_table_header($vbphrase['calendar_permissions_gcppermission']);
	print_description_row('
	...
	$calendars = $vbulletin->db->query_read("SELECT calendarid, title FROM " . TABLE_PREFIX . "calendar ORDER BY displayorder");
	$calendarcache = array();
	...
	unset($calendar);
	$vbulletin->db->free_result($calendars);

	// query forum permissions
	$calendarpermissions = $vbulletin->db->query_read("
	...
	$permscache = array();
	while ($cperm = $vbulletin->db->fetch_array($calendarpermissions))
	{
		if ($cperm['hasdata'])
		{
			$temp = array();
	...
	foreach ($calendarcache AS $calendarid => $title)
	{
		...
		echo "$indent<li><b><a name=\"calendar$calendarid\" href=\"admincalendar.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;c=$calendarid\">$title</a></b>";
		...
		foreach ($vbulletin->usergroupcache AS $usergroupid => $usergroup)
		{
			...
			echo "$indent\t<li$fp[class]" . iif($fp['calendarpermissions'] & $vbulletin->bf_ugp_calendarpermissions['canview'], '', ' style="list-style-type:circle;"') . '>' . construct_link_code($vbphrase['edit'], "calendarpermission.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;$fp[link]") . $usergroup['title'] . "</li>\n";
