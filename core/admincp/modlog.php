...
define('CVS_REVISION', '$RCSfile$ - $Revision: 74017 $');
		...
		foreach ($logs as $log)
		{
			...
			if ($log['type'])
			{
				...
				else
				{
					...
					if ($log['routeid'])
					{
						$titleurl = vB5_Route::buildUrl($log['routeid'] . '|nosession|fullurl', array('nodeid' => $log['nodeid'], 'title' => $title, 'innerPost' => $log['nodeid']), array(), '#post' . $log['nodeid']);
