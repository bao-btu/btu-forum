...
define('CVS_REVISION', '$RCSfile$ - $Revision: 71254 $');
...
if ($_REQUEST['do'] == 'emailpassword')
{
	...
	construct_hidden_code('url', $vb5_config['Misc']['admincpdir'] . "/user.php?do=find&user[email]=" . urlencode($vbulletin->GPC['email']));
...
if ($_REQUEST['do'] == 'remove')
{
	...
	echo '<p align="center">' . construct_phrase($vbphrase['if_you_want_to_prune_user_posts_first'], "thread.php?" . vB::getCurrentSession()->get('sessionurl') . "do=pruneuser&amp;f=-1&amp;u=" . $vbulletin->GPC['userid'] . "&amp;confirm=1") . '</p>';
}
...
if ($_REQUEST['do'] == 'edit' OR $_REQUEST['do'] == 'add')
{
	...
	{
		$user = vB_Api::instanceInternal('user')->fetchProfileInfo($vbulletin->GPC['userid']);
		...
		$quicklinks = array(
			"user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=editaccess&u=" . $vbulletin->GPC['userid']
				=> $vbphrase['edit_forum_permissions_access_masks'],
			"resources.php?" . vB::getCurrentSession()->get('sessionurl') . "do=viewuser&u=" . $vbulletin->GPC['userid']
				=> $vbphrase['view_forum_permissions_gcpuser'],
			"mailto:$user[email]"
		...
		{
			$quicklinks[
				"../register.php?" . vB::getCurrentSession()->get('sessionurl') . "do=requestemail&email=" . urlencode(unhtmlspecialchars($user['email'])) . '&amp;url=' . urlencode($vbulletin->options['bburl'] . '/' . $vb5_config['Misc']['admincpdir'] . '/user.php?do=edit&u=' . $vbulletin->GPC['userid'])
		...
		{
			$quicklinks[
				"subscriptions.php?" . vB::getCurrentSession()->get('sessionurl') . "do=adjust&amp;userid=" . $vbulletin->GPC['userid']
		...
		$quicklinks = array_merge(
			$quicklinks,
			array(
			"user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=emailpassword&amp;u=" . $vbulletin->GPC['userid'] . "&amp;email=" . urlencode(unhtmlspecialchars($user['email']))
				=> $vbphrase['email_password_reminder_to_user'],
			"../private.php?" . vB::getCurrentSession()->get('sessionurl') . "do=newpm&amp;u=" . $vbulletin->GPC['userid']
				=> $vbphrase['send_private_message_to_user'],
			"usertools.php?" . vB::getCurrentSession()->get('sessionurl') . "do=pmfolderstats&amp;u=" . $vbulletin->GPC['userid']
				=> $vbphrase['private_message_statistics_gcpuser'],
			"usertools.php?" . vB::getCurrentSession()->get('sessionurl') . "do=removepms&amp;u=" . $vbulletin->GPC['userid']
				=> $vbphrase['delete_all_users_private_messages'],
			"usertools.php?" . vB::getCurrentSession()->get('sessionurl') . "do=removesentpms&amp;u=" . $vbulletin->GPC['userid']
				=> $vbphrase['delete_private_messages_sent_by_user'],
			"usertools.php?" . vB::getCurrentSession()->get('sessionurl') . "do=removesentvms&amp;u=" . $vbulletin->GPC['userid']
				=> $vbphrase['delete_visitor_messages_sent_by_user'],
			"usertools.php?" . vB::getCurrentSession()->get('sessionurl') . "do=removesubs&amp;u=" . $vbulletin->GPC['userid']
				=> $vbphrase['delete_subscriptions'],
			"usertools.php?" . vB::getCurrentSession()->get('sessionurl') . "do=doips&amp;u=" . $vbulletin->GPC['userid'] . "&amp;hash=" . CP_SESSIONHASH
				=> $vbphrase['view_ip_addresses'],
			vB5_Route::buildUrl('profile|bburl', $user)
				=> $vbphrase['view_profile'],
			"../search.php?" . vB::getCurrentSession()->get('sessionurl') . "do=finduser&amp;u=" . $vbulletin->GPC['userid']. "&amp;contenttype=vBForum_Post&amp;showposts=1"
				=> $vbphrase['find_posts_by_user'],
			"admininfraction.php?" . vB::getCurrentSession()->get('sessionurl') . "do=dolist&amp;startstamp=1&amp;endstamp= " . TIMENOW . "&amp;infractionlevelid=-1&amp;u=" . $vbulletin->GPC['userid']
				=> $vbphrase['view_infractions_gcpuser'],
			'../' . $vb5_config['Misc']['modcpdir'] . '/banning.php?' . vB::getCurrentSession()->get('sessionurl') . "do=banuser&amp;u=" . $vbulletin->GPC['userid']
				=> $vbphrase['ban_user_gcpuser'],
			"user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=remove&u=" . $vbulletin->GPC['userid']
				=> $vbphrase['delete_user'],
			"socialgroups.php?" . vB::getCurrentSession()->get('sessionurl') . "do=groupsby&u=" . $vbulletin->GPC['userid']
				=> $vbphrase['view_social_groups_created_by_user'],
			)
		);

		if (intval($user['adminpermissions']) & $vbulletin->bf_ugp_adminpermissions['cancontrolpanel'] AND in_array($vbulletin->userinfo['userid'], preg_split('#\s*,\s*#s', $vb5_config['SpecialUsers']['superadmins'], -1, PREG_SPLIT_NO_EMPTY)))
		{
			$quicklinks["adminpermissions.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&u=" . $vbulletin->GPC['userid']] = $vbphrase['edit_administrator_permissions'];
		}

		$userfield = vB::getDbAssertor()->getRow('vBForum:userfield',array('userid' => $vbulletin->GPC['userid']));
	...
	if ($vbulletin->GPC['userid'])
	{
		...
		{
			var modcpurl = "<?php echo(fetch_js_safe_string($vb5_config['Misc']['modcpdir'])); ?>";
			if (url != '')
			{
	...
	if ($vbulletin->GPC['userid'])
	{
		...
		//$haschangehistory = $vbulletin->db->num_rows($userchange_list) ? true : false;
	}
	...
	print_table_header($vbphrase['profile_guser'] . ($haschangehistory ? '<span class="smallfont">' . construct_link_code($vbphrase['view_change_history'], 'user.php?' . vB::getCurrentSession()->get('sessionurl') . 'do=changehistory&amp;userid=' . $vbulletin->GPC['userid'])  . '</span>' : ''));
	...
	else
	{
		if ($user['hascustomavatar'])
		{
			...
			{
				$avatarurl = "../image.php?" . vB::getCurrentSession()->get('sessionurl') . "u=$user[userid]&amp;dateline=$user[avatardateline]";
			}
		...
		{
			$avatarurl = vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'];
		}
	}
	if ($user['hasprofilepic'])
	{
		...
		{
			$profilepicurl = "../image.php?" . vB::getCurrentSession()->get('sessionurl') . "u=$user[userid]&amp;type=profile&amp;dateline=$user[profilepicdateline]";
		}
	...
	{
		$profilepicurl = vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'];
	}
	if ($user['hassigpic'])
	{
		if ($vbulletin->options['usefileavatar'])
		...
		{
			$sigpicurl = "../image.php?" . vB::getCurrentSession()->get('sessionurl') . "u=$user[userid]&amp;type=sigpic&amp;dateline=$user[sigpicdateline]";
		}
	...
	{
		$sigpicurl = vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'];
	}

	print_label_row($vbphrase['avatar_guser'] . '<input type="image" src="' . vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'] . '" alt="" />', '<img src="' . $avatarurl . '" alt="" align="top" /> &nbsp; <input type="submit" class="button" tabindex="1" name="modifyavatar" value="' . $vbphrase['change_avatar'] . '" />');
	print_label_row($vbphrase['signature_picture_guser'] . '<input type="image" src="' . vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'] . '" alt="" />', '<img src="' . $sigpicurl . '" alt="" align="top" /> &nbsp; <input type="submit" class="button" tabindex="1" name="modifysigpic" value="' . $vbphrase['change_signature_picture'] . '" />');
	print_table_break('', $INNERTABLEWIDTH);
	...
	if ($banreason)
	{
		...
		$row = array($vbphrase['ban_reason'], (!empty($banreason['reason']) ? $banreason['reason'] : $vbphrase['n_a']), construct_link_code($vbphrase['lift_ban'], "../" . $vb5_config['Misc']['modcpdir'] . "/banning.php?" . vB::getCurrentSession()->get('sessionurl') . "do=liftban&amp;userid=" . $user['userid']));
		print_cells_row($row);
	...
	// INFRACTIONS section
	print_table_header($vbphrase['infractions'] . '<span class="smallfont">' . construct_link_code($vbphrase['view'], "admininfraction.php?" . vB::getCurrentSession()->get('sessionurl') . "do=dolist&amp;startstamp=1&amp;endstamp= " . TIMENOW . "&amp;infractionlevelid=-1&amp;u= " . $vbulletin->GPC['userid']) . '</span>');
...
if ($_POST['do'] == 'update')
{
	$vbulletin->input->clean_array_gpc('p', array(
		...
		'modifycss'         => vB_Cleaner::TYPE_NOCLEAN,
		'subscriptionlogid' => vB_Cleaner::TYPE_ARRAY_KEYS_INT,
//we no longer appear to need old user -- the api handles it via a DB lookup.
//		'olduser'           => vB_Cleaner::TYPE_BINARY
	));
	...
	{
		//we can only print one error, so print the first one
		call_user_func_array('print_stop_message', $userid['errors'][0]);
	}
	...
	else
	{
		...
		if (!$handled)
		{
			...
			if ($userdata->insertedadmin)
			{
				$args['insertedadmin'] = 1;
			}
		}
	}
...
if ($_REQUEST['do'] == 'editaccess')
{
	...
	$accessarray = vB_Api::instanceInternal('user')->fetchAccess($vbulletin->GPC['userid']);
/*	$user = $vbulletin->db->query_first("SELECT username, options FROM " . TABLE_PREFIX . "user WHERE userid = " . $vbulletin->GPC['userid']);

	$accesslist = $vbulletin->db->query_read("SELECT * FROM " . TABLE_PREFIX . "access WHERE userid = " . $vbulletin->GPC['userid']);

	echo '<h1>$vbulletin->db->numrows($accesslist) = ' . $vbulletin->db->num_rows($accesslist) . '<br />user.hasaccessmask = ' . ($user['options'] & $vbulletin->bf_misc_useroptions['hasaccessmask'] ? 'yes' : 'no') . '</h1>';

	while ($access = $vbulletin->db->fetch_array($accesslist))
	{
	...
	print_cells_row(array($vbphrase['channel'], $vbphrase['allow_access_to_channel']), 0, 'thead', -2);
	print_label_row('&nbsp;', '
...
if ($_POST['do'] == 'updateaccess')
{
	...
	print_stop_message2('saved_access_masks_successfully', 'user', array('do' => 'edit', 'u' => $vbulletin->GPC['userid']));
}
...
if ($_REQUEST['do'] == 'modify')
{
	...
	if ($vbulletin->GPC['userid'])
	{
		...
		construct_hidden_code('userid', $vbulletin->GPC['userid']);
		print_description_row(
			construct_link_code($vbphrase['view_profile'], "user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;u=" . $vbulletin->GPC['userid']) .
			iif($vbulletin->GPC['insertedadmin'], '<br />' . construct_link_code('<span style="color:red;"><strong>' . $vbphrase['update_or_add_administration_permissions'] . '</strong></span>', "adminpermissions.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;u=" . $vbulletin->GPC['userid']))
		);
		print_table_footer();
	}
	...
	print_description_row('<img src="' . vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'] . '" alt="" width="1" height="2" />', 0, 2, 'thead');
...
if ($_REQUEST['do'] == 'find2' AND defined('DONEFIND'))
{
	...
	function js_usergroup_jump(userinfo)
	{
		...
		if (value != "")
		{
			...
			window.location = "user.php?<?php echo vB::getCurrentSession()->get('sessionurl_js'); ?>do=" + page;
		}
	}
	</script>
	<?php
	...
	print_table_header(
		construct_phrase(
			...
			$vbulletin->GPC['limitstart'] ? $vbulletin->GPC['limitstart'] : 1,
...
if ($_REQUEST['do'] == 'moderate')
{
	...
	<?php
		...
		{
			$cell = array();
			$cell[] = "<a href=\"user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;u=$user[userid]\" target=\"_user\"><b>$user[username]</b></a>";
			$cell[] = "<a href=\"mailto:$user[email]\">$user[email]</a>";
			$cell[] = "<a href=\"usertools.php?" . vB::getCurrentSession()->get('sessionurl') . "do=doips&amp;depth=2&amp;ipaddress=$user[ipaddress]&amp;hash=" . CP_SESSIONHASH . "\" target=\"_user\">$user[ipaddress]</a>";
			$cell[] = "
	...
	else
	{
		...
		$users = vB::getDbAssertor()->assertQuery('user', array(
			...
			vB_dB_Query::CONDITIONS_KEY => array(
				array('field' => 'userid', 'value' => array_keys($vbulletin->GPC['validate']), 'operator' => vB_dB_Query::OPERATOR_EQ)
			)
		));
			...
			{ // validated
				...
				if ($vbulletin->options['welcomepm'] AND $fromuser AND !$user['posts'])
				{
					if (!isset($evalpm_validated["$user[languageid]"]))
					{
						...
						//was cached on the first pass -- so we make sure that we load and cache all language version
						//in case the second user has a different language from the first
						$phraseAux = vB_Api::instanceInternal('phrase')->fetch(array('welcomepm_gemailbody', 'welcomepm_gemailsubject'));
						$text_message = $phraseAux['welcomepm_gemailbody'];
						$text_subject = $phraseAux['welcomepm_gemailsubject'];
					...
					// Legacy Hook 'private_insertpm_process' Removed //
					// Legacy Hook 'private_insertpm_complete' Removed //
				}
			}
			else if ($status == -1)
			{ // deleted
...
if ($_REQUEST['do'] == 'prune_updateposts')
{
	...
	$users = vB::getDbAssertor()->assertQuery('userFind', array(
		vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_METHOD,
		'filters' => array(array('field' => 'user.userid', 'value' => $userids, 'operator' => vB_dB_Query::OPERATOR_EQ)),
		'exceptions' => array(),
		'orderby' => 'userid',
		'direction' => 'ASC',
		'limitstart' => $vbulletin->GPC['startat'],
		'limitnumber' => 50,
	));
	if ($users AND $users->valid())
	{
		foreach ($users as $user)
		{
			...
			vB::getDbAssertor()->assertQuery('thread',
				array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_UPDATE,
					'postuserid' =>0,
					'postusername' => $user['username'],
					vB_dB_Query::CONDITIONS_KEY => array(
						array('field' => 'postuserid', 'value' => $user['userid'], 'operator' => vB_dB_Query::OPERATOR_EQ)
					)
				)
			);
			vB::getDbAssertor()->assertQuery('post',
				array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_UPDATE,
					'userid' =>0,
		...
		$args = array();
		parse_str(vB::getCurrentSession()->get('sessionurl'),$args);
		$args['do'] = 'prune_updateposts';
		$args['startat'] = $vbulletin->GPC['startat'];
		print_cp_redirect2('user', $args, 0);
	...
	else
	{
		...
		vB::getDbAssertor()->delete('usertextfield',array('userid' => $userids));
		vB::getDbAssertor()->delete('userfield',array('userid' => $userids));
		vB::getDbAssertor()->delete('user',array('userid' => $userids));
		// Legacy Hook 'useradmin_prune' Removed //
	...
	if (!empty($vbulletin->GPC['users']))
	{
		...
		try
		{
			...
			if ($vbulletin->GPC['dowhat'] == 'delete')
			{
				...
				$args['do'] = 'prune_updateposts';
				$args['startat'] = 0;
				print_cp_redirect2('user', $args, 1);
				exit;
			}
...
if ($_REQUEST['do'] == 'pruneusers')
{
	...
	if (!empty($vbulletin->GPC['order']))
	{
		try
		{
			$usersPrune = vB::getDbAssertor()->assertQuery('fetchPruneUsers',
		...
		{
			print_stop_message2($e->getMessage());
		}
		...
		if ($usersPrune AND $usersPrune->valid())
		{
			...
			foreach($usersPrune as $user)
			{
				$cell = array();
				$cell[] = $user['userid'];
				$cell[] = "<a href=\"user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&u=$user[userid]\" target=\"_blank\">$user[username]</a><br /><span class=\"smallfont\">$user[title]" . ($user['moderatorid'] ? ", " . $vbphrase['moderator'] : "" ) . "</span>";
				$cell[] = "<a href=\"mailto:$user[email]\">$user[email]</a>";
				$cell[] = vb_number_format($user['posts']);
		...
		if ($userchange_list)
		{
			...
			foreach ($userchange_list as $userchange)
			{
				if (!$printed['header'])
				{
					...
					print_table_header($vbphrase['view_change_history'] . ' <span class="normal"><a href="user.php?' . vB::getCurrentSession()->get('sessionurl') . 'do=edit&amp;userid=' . $userchange['userid'] . '">' . $userchange['username'] . '</a>', 3);
