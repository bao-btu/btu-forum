...
define('CVS_REVISION', '$RCSfile$ - $Revision: 70436 $');
...
if ($_POST['do'] == 'update')
{
	...
	if ($vbulletin->GPC['dostyleid'] != -1)
	{
		...
		$templateids = unserialize($style['templatelist']);
		$conditions[] = array('field' => 'templateid','value' => $templateids, 'operator' => vB_dB_Query::OPERATOR_EQ);

	}
	else
	{
		$conditions[] = array('field' => 'styleid','value' => -1, 'operator' => vB_dB_Query::OPERATOR_EQ);
	}

	$templates = $assertor->assertQuery('template',
		array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT,
			vB_dB_Query::CONDITIONS_KEY => $conditions
		)
	);

	if ($templates AND $templates->valid())
	{
	...
	if ($changed)
	{
		...
		print_cp_redirect2($file, $args, 1);
	}
	else
	{
		print_stop_message2('nothing_to_do',$file, $args);
	}
}
...
if ($_POST['do'] == 'insert')
{
	...
	$existing = $assertor->getRow('template',
		array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT,
				vB_dB_Query::CONDITIONS_KEY => array(
					'styleid' => $vbulletin->GPC['dostyleid'],
					'templatetype' => 'replacement',
...
function print_replacements($parentid = -1, $indent = "\t")
{
	...
	foreach ($stylecache[$parentid] AS $holder)
	{
		...
		foreach ($holder AS $styleid => $style)
		{
			...
			$templates = $assertor->getRows('getReplacementTemplates',	array('templateids' => $templateids));
