...
define('CVS_REVISION', '$RCSfile$ - $Revision: 71338 $');
define('NOZIP', 1);
...
if ($_POST['do'] == 'doupload')
{
	...
	{
		print_diagnostic_test_result(0, construct_phrase($vbphrase['no_file_uploaded_and_no_local_file_found_gcpglobal'], $vbphrase['test_cannot_continue']));
	}
...
if ($_POST['do'] == 'dosysinfo')
{
	...
	switch ($vbulletin->GPC['type'])
	{
		...
		default:
			...
			{
				print_stop_message2(array('table_status_not_available',  $mysqlversion['version']));
			}
...
if ($_POST['do'] == 'doversion')
{
	...
	if ($handle)
	{
		...
		$md5_sum_versions = array('vbulletin' => '5.0.3');
		$file_software_assoc = array();
		$scanned_md5_files = array();
		$ignored_files = array('/includes/config.php', '/includes/config.php.new', '/install/install.php', '/install/makeconfig.php', '/includes/version_vbulletin.php');
		...
		while ($file = readdir($handle))
		{
			if (preg_match('#^md5_sums_.+$#siU', $file, $match))
			{
				...
				foreach ($md5_sums AS $key => $val)
				{
					...
					if ($vb5_config['Misc']['admincpdir'] !== 'admincp' AND $admin_dir === 0)
					{
						$key = substr_replace($key, $vb5_config['Misc']['admincpdir'], 1, strlen('admincp'));
					}
					else if ($vb5_config['Misc']['modcpdir'] !== 'modcp' AND $mod_dir === 0)
					{
						$key = substr_replace($key, $vb5_config['Misc']['modcpdir'], 1, strlen('modcp'));
					}
		...
		{
			print_stop_message2('unable_to_read_md5_sums');
		}
