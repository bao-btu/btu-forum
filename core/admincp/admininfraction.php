...
define('CVS_REVISION', '$RCSfile$ - $Revision: 72357 $');
...
if ($_REQUEST['do'] == 'dolist')
{
	...
	if ($infractions AND $infractions->valid())
	{
		...
		{
			$cell = array();
			$cell[] = "<a href=\"user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;u=$infraction[infracteduserid]\"><b>$infraction[username]</b></a>";
			...
			if (!empty($infraction['infractednodeid']) AND !$infraction['postdeleted'])
			{
				...
				$postlink = vB5_Route::buildUrl($infractedNode['routeid'] . '|nosession|fullurl', array('nodeid' => $infractedNode['nodeid'], 'title' => $infractedNode['title']), array('p' => $infractedNode['nodeid'])) . '#post' . $infractedNode['nodeid'];
			}
