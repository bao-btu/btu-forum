...
define('CVS_REVISION', '$RCSfile$ - $Revision: 75483 $');
define('NOZIP', 1);

// #################### PRE-CACHE TEMPLATES AND DATA ######################
global $phrasegroups, $specialtemplates, $vbphrase, $vbulletin;
$phrasegroups = array('maintenance', 'user', 'infraction');
...
{
	$result = vB_Api::instance('search')->emptyIndex();
...
{
	print_form_header('misc', 'dorebuildindex');
	print_table_header($vbphrase['confirm_deletion_gcpglobal']);
...
if ($_POST['do'] == 'dorebuildindex')
{
	...
	parse_str(vB::getCurrentSession()->get('sessionurl'),$args);
	$args['do'] = 'chooser';
	$result = vB_Api::instance('search')->reIndexAll(true);
	if (isset($result['errors']))
	{
		print_stop_message2($result['errors'][0]);
	}
	...
	{
		print_stop_message2('rebuilt_search_index_not_implemented', 'misc', $args);
	}
}
...
if ($_REQUEST['do'] == 'doindextypes')
{
	...
	echo '<p>' . $vbphrase['building_search_index'] . ' ' . $indextype . ' ' .
				 ...
				 ($startat + $perpage) . '</p>';
	vbflush();
	$hasmore = vB_Api::instance('search')->indexRange($startat, $perpage, $filters);
...
if ($_REQUEST['do'] == 'addmissingkeywords')
{
	...
	{
		$gotsome = true;
		$threadinfo = vB_Api::instance('node')->getNode($thread['nodeid']);
		if (isset($threadinfo['errors']))
		{
			print_stop_message2($threadinfo['errors'][0]);
		}
