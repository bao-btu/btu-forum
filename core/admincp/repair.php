...
print_cp_header($vbphrase['repair_optimize_tables_gsql']);
...
function exec_sql_table_check($table)
{
	...
	if ($vbulletin->GPC['repairtables'])
	{
		...
		while ($msg = $vbulletin->db->fetch_array($checkmsgs, vB_Database::DBARRAY_NUM))
		{
			if ($msg[2] == 'error')
			{
	...
	if ($vbulletin->GPC['optimizetables'] AND !$error AND !$error2 AND !$nooptimize)
	{
		...
		while ($msg = $vbulletin->db->fetch_array($opimizemsgs, vB_Database::DBARRAY_NUM))
		{
			if ($msg[2] == 'error')
			{
				$error = 1;
...
if ($_POST['do'] == 'dorepair')
{
	$vbulletin->input->clean_array_gpc('p', array(
		'tableserial'    => vB_Cleaner::TYPE_STR,
		'tablelist'      => vB_Cleaner::TYPE_ARRAY_STR,
		'optimizetables' => vB_Cleaner::TYPE_BOOL,
		'repairtables'   => vB_Cleaner::TYPE_BOOL,
		'converttables'  => vB_Cleaner::TYPE_BOOL,
		'isamtablelist'  => vB_Cleaner::TYPE_ARRAY_STR,
	));
...
if ($_REQUEST['do'] == 'list')
{
	...
	print_table_header($vbphrase['repair_optimize_tables_gsql'], 5);
	$headings = array();
...
if ($_REQUEST['do'] == 'fixunique')
{
	$vbulletin->input->clean_array_gpc('r', array(
		'tableid' => vB_Cleaner::TYPE_UINT
	));

	if (!file_exists(DIR . '/install/mysql-schema.php'))
	{
		print_stop_message2('restore_mysql_schema');
	}
	...
	{
		global $vbphrase, $vbulletin, $uniquetables;
		...
		$unique_keys = array();
		$keys = array();
		$checkindexes = $vbulletin->db->query_write("SHOW KEYS FROM " . TABLE_PREFIX . "$table[name]");
		while ($checkindex = $vbulletin->db->fetch_array($checkindexes))
		{
		...
		$vbulletin->db->free_result($checkindexes);
		...
		else
		{
			...
			$findquery = "SELECT $fields, COUNT(*) AS occurences
				...
				GROUP BY $fields
				HAVING occurences > 1";
			$dupes = $vbulletin->db->query_write($findquery);

			if ($numdupes = $vbulletin->db->num_rows($dupes))
			{
				echo "<li>" . construct_phrase($vbphrase['found_x_duplicate_record_occurences'], "<strong>$numdupes</strong>") . "<ol>";

				while ($dupe = $vbulletin->db->fetch_array($dupes))
				{
					$cond = array();
					...
					foreach ($dupe AS $fieldname => $field)
					{
						...
						{
							$cond[] = "$fieldname = " . iif(is_numeric($field), $field, "'" . $vbulletin->db->escape_string($field) . "'");
						}
					}
					...
					$vbulletin->db->query_write($dupesquery);
					echo "<li>$vbphrase[deleted_duplicate_occurence]</li>";
				}
				$vbulletin->db->free_result($dupes);
			...
			if (isset($keys["$table[keyname]"]))
			{
				...
				$vbulletin->db->query_write($killindexquery);
			...
			$vbulletin->db->query_write($createindexquery);
			echo "$vbphrase[done]</li>";
