...
define('CVS_REVISION', '$RCSfile$ - $Revision: 75651 $');
...
if ($_REQUEST['do'] == 'add' OR $_REQUEST['do'] == 'edit')
{
		...
		$(document).ready(function() {
			...
			var resetCurrencySelect = function(container) {
				...
				$('select.currency', container).each(function() {
					...
					// Update input name
					selectbox.closest('div.pricerow').find('input.cost').attr('name', 'sub[time][' + selectbox.attr('data-index') + '][cost][' + selectval + ']');
			...
			$(document).on('change', 'select.currency', function() {
				$(this).closest('div.pricerow').find('input.cost').attr('name', 'sub[time][' + $(this).attr('data-index') + '][cost][' + $(this).val() + ']');
	...
	<?php
	print_form_header('subscriptions', 'update');
	...
	print_yes_no_row($vbphrase['display_subscription_during_registration'], 'newoptions[regshow]', (isset($sub['newoptions']['regshow'])?$sub['newoptions']['regshow']:1));
	...
	// TODO: Commented until VBV-1238 is fixed
	/*print_table_break();
	print_table_header($vbphrase['channels']);
...
{
	print_delete_confirmation('vBForum:subscription', $vbulletin->GPC['subscriptionid'],
