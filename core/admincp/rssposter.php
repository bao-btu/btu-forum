...
define('CVS_REVISION', '$RCSfile$ - $Revision: 72473 $');
...
if ($_REQUEST['do'] == 'edit')
{
	...
	$icons = array();
	$icons_result = $assertor->getRows('vBForum:icon', array(), array('imagecategoryid', 'displayorder'));
	...
	else
	{
		...
		foreach ($feeds AS $rssfeedid => $feed)
		{
			...
			print_cells_row(array(
				...
				"<div><a href=\"rssposter.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;rssfeedid=$feed[rssfeedid]\" title=\"" . htmlspecialchars_uni($feed['url']) . "\"><strong>$feed[title]</strong></a></div>
				<div class=\"smallfont\"><a href=\"" . htmlspecialchars_uni($feed['url']) . "\" target=\"feed\">$x[host]</a></div>",
				"<div><a href=\"forum.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;nodeid=$feed[nodeid]\">$feed[channeltitle]</a></div>
				<div class=\"smallfont\"><a href=\"user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;userid=$feed[userid]\">$feed[username]</a></div>",
		...
		print_submit_row(
			...
			"
				<input type=\"button\" class=\"button\" value=\"$vbphrase[run_scheduled_task_now]\" onclick=\"window.location='cronadmin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=runcron&amp;varname=rssposter'\" />
