...
define('CVS_REVISION', '$RCSfile$ - $Revision: 74956 $');
...
// Legacy Hook 'admin_profilefield_start' Removed //
...
if ($_POST['do'] == 'removecat')
{
	...
	"))
	{
		...
		print_stop_message2('deleted_profile_field_category_successfully', 'profilefield', array('do'=>'modifycats'));
	}
	else
	{
		$_REQUEST['do'] = 'modifycats';
...
if ($_POST['do'] == 'updatecat')
{
	...
	if (!$_POST['profilefieldcategoryid'])
	{
		...
		$vbulletin->db->query_write("
			...
			VALUES
				(NULL, " . $vbulletin->GPC['displayorder'] . ", '" . $vbulletin->db->escape_string($vbulletin->GPC['location']) . "', " . intval($vbulletin->GPC['allowprivacy']) . ")
		");

		$vbulletin->GPC['profilefieldcategoryid'] = intval($vbulletin->db->insert_id());
	}
	else
	{
		...
		$vbulletin->db->query_write("
			UPDATE " . TABLE_PREFIX . "profilefieldcategory SET
				...
				location = '" . $vbulletin->db->escape_string($vbulletin->GPC['location']) . "',
	...
	// redirect to category list page
	print_stop_message2(array('saved_x_successfully',  $vbulletin->GPC['title']), 'profilefield', array('do'=>'modifycats'));
}
...
if ($_REQUEST['do'] == 'addcat' OR $_REQUEST['do'] == 'editcat')
{
	...
	if ($_REQUEST['do'] == 'editcat' AND $pfc = $vbulletin->db->query_first("SELECT * FROM " . TABLE_PREFIX . "profilefieldcategory WHERE profilefieldcategoryid = " . $vbulletin->GPC['profilefieldcategoryid']))
	{
	...
	$trans_link = "phrase.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&fieldname=cprofilefield&t=1&varname=";
	...
	//@TODO: location and allow privacy will be used again when the profile categories will be fixed, atm they are not changing anything on frontend
...
if ($_POST['do'] == 'displayordercats')
{
	...
	if (!empty($vbulletin->GPC['order']))
	{
		...
		print_stop_message2('saved_display_order_successfully', 'profilefield', array('do'=>'modifycats'));
	}
	else
	{
		$_REQUEST['do'] = 'modifycats';
...
if ($_POST['do'] == 'displayorder')
{
	...
	if (!empty($vbulletin->GPC['order']))
	{
		...
		print_stop_message2('saved_display_order_successfully', 'profilefield', array('do'=>'modify'));
	}
	else
	{
		$_REQUEST['do'] = 'modify';
...
if ($_POST['do'] == 'update')
{
	...
	if ($vbulletin->GPC['profilefield']['maxlength'] < 1)
	{
		...
		{
			print_stop_message2('must_have_positive_maxlength');
		}
	...
	if ($vbulletin->GPC['type'] == 'select' OR $vbulletin->GPC['type'] == 'radio' OR (($vbulletin->GPC['type'] == 'checkbox' OR $vbulletin->GPC['type'] == 'select_multiple') AND empty($vbulletin->GPC['profilefieldid'])))
	{
		...
		{
			print_stop_message2('can_not_duplicate_options');
		}

		if (sizeof($data) > 31 AND ($vbulletin->GPC['type'] == 'checkbox' OR $vbulletin->GPC['type'] == 'select_multiple'))
		{
			print_stop_message2(array('too_many_profile_field_options',  sizeof($data)));
		}
	...
	$vbulletin->GPC['profilefield']['profilefieldcategoryid'] = $vbulletin->cleaner->clean($vbulletin->GPC['profilefield']['profilefieldcategoryid'], vB_Cleaner::TYPE_UINT);

	$pfcs = array();
	$pfcs_result = $vbulletin->db->query_read("SELECT * FROM " . TABLE_PREFIX . "profilefieldcategory");
	while ($pfc = $vbulletin->db->fetch_array($pfcs_result))
	{
...
if ($_REQUEST['do'] == 'add' OR $_REQUEST['do'] == 'edit')
{
	...
	if ($_REQUEST['do'] == 'add')
	{
		...
		print_table_header($vbphrase['add_new_user_profile_field_gprofilefield'] . " <span class=\"normal\">" . $types["{$vbulletin->GPC['type']}"] . "</span>", 2, 0);

	}
	else
	{
		$profilefield = vB::getDbAssertor()->getRow('vBForum:profilefield', array('profilefieldid' => $vbulletin->GPC['profilefieldid']));
	...
	{
		print_input_row($vbphrase['title'] . '<dfn>' . construct_link_code($vbphrase['translations'], "phrase.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&fieldname=cprofilefield&varname=$profilefield[titlevarname]&t=1", 1)  . '</dfn>', 'title', $profilefield['title']);
	}
	else
	{
	...
	{
		print_textarea_row($vbphrase['description_gcpglobal'] . $extra . '<dfn>' . construct_link_code($vbphrase['translations'], "phrase.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&fieldname=cprofilefield&varname=$profilefield[descvarname]&t=1", 1)  . '</dfn>', 'description', $profilefield['description']);
	}
	else
	{
		print_textarea_row($vbphrase['description_gcpglobal'] . $extra, 'description');
	}

	$pfcs = array(0 => '(' . $vbphrase['uncategorized_gprofilefield'] . ')');
	$pfcs_result = $vbulletin->db->query_read("SELECT * FROM " . TABLE_PREFIX . "profilefieldcategory ORDER BY displayorder");
	while ($pfc = $vbulletin->db->fetch_array($pfcs_result))
	{
		$pfcs["$pfc[profilefieldcategoryid]"] = $vbphrase['category' . $pfc['profilefieldcategoryid'] . '_title'];
	}
	$vbulletin->db->free_result($pfcs_result);
	...
	if ($vbulletin->GPC['type'] == 'checkbox')
	{
		...
		{
			print_label_row($vbphrase['fields'], '<input type="image" src="' . vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'] . '"><input type="submit" class="button" value="' . $vbphrase['modify'] . '" tabindex="1" name="modifyfields">');
		}
	}
	...
	print_select_row($vbphrase['which_page_displays_option'], 'profilefield[form]', array(
		...
		"$vbphrase[options]: $vbphrase[other_gprofilefield]"
	), $profilefield['form']);
...
if ($_REQUEST['do'] == 'renamecheckbox')
{
	...
	$boxdata = $vbulletin->db->query_first("
		SELECT data,type
...
if ($_POST['do'] == 'dorenamecheckbox')
{
	...
	if (!empty($vbulletin->GPC['newfield']))
	{
		...
		foreach ($data AS $index => $value)
		{
			...
			{
				print_stop_message2(array('this_is_already_option_named_x',  $value));
			}
		}
	...
	print_stop_message2(array('saved_option_x_successfully',  $vbulletin->GPC['newfield']), 'profilefield', array('do'=>'modifycheckbox', 'profilefieldid' => $vbulletin->GPC['profilefieldid']));
}
...
if ($_POST['do'] == 'dodeletecheckbox')
{
	...
	print_stop_message2('deleted_option_successfully', 'profilefield', array('do'=>'modifycheckbox', 'profilefieldid' => $vbulletin->GPC['profilefieldid']));
}
...
if ($_POST['do'] == 'addcheckbox')
{
	...
	if (!empty($vbulletin->GPC['newfield']))
	{
		...
		print_stop_message2('saved_option_successfully', 'profilefield', array('do'=>'modifycheckbox', 'profilefieldid' => $vbulletin->GPC['profilefieldid']));
	}
	else
	{
...
{
	$boxdata = vB::getDbAssertor()->getRow('vBForum:profilefield', array('profilefieldid' => $vbulletin->GPC['profilefieldid']));
	...
	if ($boxdata['data'] != '')
	{
		...
		foreach ($data AS $index => $value)
		{
			...
			if ($index != 1)
			{
				$moveup = "<a href=\"profilefield.php?" . vB::getCurrentSession()->get('sessionurl') . "profilefieldid=" . $vbulletin->GPC['profilefieldid'] . "&do=movecheckbox&direction=up&id=$index\"><img src=\"' . $vbulletin->options['bburl'] . '/cpstyles/" . $vbulletin->options['cpstylefolder'] . "/move_up.gif\" border=\"0\" /></a> ";
			}
			else
			{
				$moveup = '<img src="' . vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'] . '" width="11" border="0" alt="" /> ';
			}
			if ($index != sizeof($data))
			{
				$movedown = "<a href=\"profilefield.php?" . vB::getCurrentSession()->get('sessionurl') . "profilefieldid=" . $vbulletin->GPC['profilefieldid'] . "&do=movecheckbox&direction=down&id=$index\"><img src=\"' . $vbulletin->options['bburl'] . '/cpstyles/" . $vbulletin->options['cpstylefolder'] . "/move_down.gif\" border=\"0\" /></a> ";
			}
			else
			{
				unset($movedown);
			...
			construct_link_code($vbphrase['rename_gprofilefield'], "profilefield.php?do=renamecheckbox&profilefieldid=" . $vbulletin->GPC['profilefieldid'] . "&id=$index")
			."</td><td>".
			iif(sizeof($xxxdata) > 1, construct_link_code($vbphrase['move_gcpglobal'], "profilefield.php?do=movecheckbox&profilefieldid=" . $vbulletin->GPC['profilefieldid'] . "&id=$index"), '')
			. "</td><td>".
	...
	print_form_header('', '');
	print_table_header(construct_phrase($vbphrase['x_y_id_z'], $vbphrase['user_profile_field'], construct_link_code($vbphrase['field' . $vbulletin->GPC['profilefieldid'] . '_title'], "profilefield.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;profilefieldid=" . $vbulletin->GPC['profilefieldid']), $vbulletin->GPC['profilefieldid']));
	print_table_break();
...
if ($_POST['do'] == 'kill')
{
	...
	print_stop_message2('deleted_user_profile_field_successfully', 'profilefield', array('do'=>'modify'));
}
...
if ($_REQUEST['do'] == 'modify')
{
	...
	if ($vbulletin->db->num_rows($profilefields))
	{
		$forms = array(
		...
		$optionfields = array(
			'required'   => $vbphrase['required_gprofilefield'],
						...
						foreach ($fields["$formid"]["$pfcid"] AS $displayorder => $profilefields)
						{
							foreach ($profilefields AS $_profilefieldid => $profilefield)
							{
								...
								<tr>
									...
									<td class=\"$bgclass\">" .
									construct_link_code($vbphrase['edit'], "profilefield.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;profilefieldid=$_profilefieldid") .
									construct_link_code($vbphrase['delete'], "profilefield.php?" . vB::getCurrentSession()->get('sessionurl') . "do=remove&profilefieldid=$_profilefieldid") .
									"</td>
								</tr>";
							}
						}
