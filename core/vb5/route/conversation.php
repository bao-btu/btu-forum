		...
		if (!isset($this->canonicalRoute))
		{
			...
			try {
				...
				if (isset($queryParameters['p']))
				{
					...
					if (isset($user['maxposts']) && $user['maxposts']!=-1)
					{
						$resultsPerPage = $user['maxposts'];
					}
					else
					{
					...
					$pagenumber = vB_Api::instanceInternal('search')->getPageNumber($queryParameters['p'], $node, $resultsPerPage, 1, 'ASC', 1);
					$this->arguments['pagenum'] = $pagenumber;
					unset($queryParameters['p']);
				}
			} catch (Exception $e) {
				// if the hack to get page number failed, just abort so the original logic can address it
			}
