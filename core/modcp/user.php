...
define('CVS_REVISION', '$RCSfile$ - $Revision: 69031 $');
...
if ($_REQUEST['do'] == 'findnames')
{
	...
	$caneditreputation = iif(can_moderate(0, 'caneditreputation'), true);
	...
	if ($vbulletin->db->num_rows($users) > 0)
	{
		...
		while ($user = $vbulletin->db->fetch_array($users))
		{
			...
			{
				$cell[] = '<span class="smallfont">' . construct_link_code($vbphrase['ban_user'], 'banning.php?' . vB::getCurrentSession()->get('sessionurl') . "do=banuser&amp;u=$user[userid]") . '</span>';
			}
			elseif ($canunbanusers AND $user['inbannedgroup'])
			{
				$cell[] = '<span class="smallfont">' . construct_link_code($vbphrase['lift_ban'], 'banning.php?' . vB::getCurrentSession()->get('sessionurl') . "do=liftban&amp;u=$user[userid]") . '</span>';
			}
			else
			{
				$cell[] = '';
			...
			$cell[] = iif($canviewprofile, '<span class="smallfont">' . construct_link_code($vbphrase['view_profile'], 'user.php?' . vB::getCurrentSession()->get('sessionurl') . "do=viewuser&amp;u=$user[userid]") . '</span>');
			$cell[] = iif($caneditsigs, '<span class="smallfont">' . construct_link_code($vbphrase['change_signature'], 'user.php?' . vB::getCurrentSession()->get('sessionurl') . "do=editsig&amp;u=$user[userid]") . '</span>');

//these function no longer work, so hide them.
//			$cell[] = iif($caneditavatar, '<span class="smallfont">' . construct_link_code($vbphrase['change_avatar'], 'user.php?' . vB::getCurrentSession()->get('sessionurl') . "do=avatar&amp;u=$user[userid]") . '</span>');
//			$cell[] = iif($caneditprofilepic, '<span class="smallfont">' . construct_link_code($vbphrase['change_profile_picture'], 'user.php?' . vB::getCurrentSession()->get('sessionurl') . "do=profilepic&amp;u=$user[userid]") . '</span>');
			$cell[] = iif($caneditreputation, '<span class="smallfont">' . construct_link_code($vbphrase['edit_reputation_gcpuser'], 'user.php?' . vB::getCurrentSession()->get('sessionurl') . "do=reputation&amp;u=$user[userid]") . '</span>');
...
if ($_REQUEST['do'] == 'viewuser')
{
	...
	print_textarea_row($vbphrase['signature'] . iif(can_moderate(0, 'caneditsigs'), '<br /><br />' . construct_link_code($vbphrase['edit_signature'], 'user.php?' . vB::getCurrentSession()->get('sessionurl') . "do=editsig&amp;u=$user[userid]")), 'signature', $user['signature'], 8, 45, 1, 0);
	...
	print_label_row($vbphrase['avatar'] . iif(can_moderate(0, 'caneditavatar'), '<br /><br />' . construct_link_code($vbphrase['edit_avatar'], 'user.php?' . vB::getCurrentSession()->get('sessionurl') . "do=avatar&amp;u=$user[userid]")) . '<input type="image" src="' . vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'] . '" alt="" />','<img src="' . $avatarurl . '" alt="" align="top" />');
	print_label_row($vbphrase['profile_picture'] . iif(can_moderate(0, 'caneditprofilepic'), '<br /><br />' . construct_link_code($vbphrase['edit_profile_picture_gcpuser'], 'user.php?' . vB::getCurrentSession()->get('sessionurl') . "do=profilepic&amp;u=$user[userid]")) . '<input type="image" src="' . vB::getDatastore()->getOption('bburl') . '/' . $vbulletin->options['cleargifurl'] . '" alt="" />','<img src="' . $profilepicurl . '" alt="" align="top" />');
	print_table_break('', $INNERTABLEWIDTH);
	...
	print_yes_no_row($vbphrase['display_email_gcpuser'], 'options[showemail]', $user[showemail]);
...
if ($_REQUEST['do'] == 'profilepic')
{
	...
	print_table_header($vbphrase['edit_profile_picture_gcpuser']);
...
if ($_POST['do'] == 'updateprofilepic')
{
	...
	if ($vbulletin->GPC['useprofilepic'])
	{
		...
		$upload->data = new vB_DataManager_Userpic_Profilepic($vbulletin, vB_DataManager_Constants::ERRTYPE_CP);
	...
	else
	{
		...
		$userpic = new vB_Datamanager_Userpic_Profilepic($vbulletin, vB_DataManager_Constants::ERRTYPE_CP);
	...
	print_modcp_stop_message2('saved_profile_picture_successfully', $file, $args);
...
if ($_REQUEST['do'] == 'viewjoinrequests')
{
	...
	{
		$userlink = '<a href="../' . $vb5_config['Misc']['admincpdir'] . '/user.php?' . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;u=%d\" target=\"_blank\">%s</a>";
		$grouplink = '../' . $vb5_config['Misc']['admincpdir'] . '/usergroup.php?' . vB::getCurrentSession()->get('sessionurl') . "do=viewjoinrequests&amp;usergroupid=%d";
	}
	else
	{
		$userlink = '<a href="user.php?' . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;u=%d\" target=\"_blank\">%s</a>";
		$grouplink = '../joinrequests.php?' . vB::getCurrentSession()->get('sessionurl') . "usergroupid=%d";
	}

	// get array of all usergroup leaders
	$bbuserleader = array();
	$leaders = array();
	$groupleaders = $vbulletin->db->query_read("
		SELECT ugl.*, user.username
		FROM " . TABLE_PREFIX . "usergroupleader AS ugl
		INNER JOIN " . TABLE_PREFIX . "user AS user USING(userid)
	");
	while ($groupleader = $vbulletin->db->fetch_array($groupleaders))
	{
	...
	unset($groupleader);
	$vbulletin->db->free_result($groupleaders);
	...
	unset($request);
	$vbulletin->db->free_result($requests);
	...
	foreach ($vbulletin->usergroupcache AS $usergroupid => $usergroup)
	{
		if ($usergroup['ispublicgroup'] AND in_array($usergroupid, $bbuserleader))
		{
			print_cells_row(array(
				...
				construct_link_code($vbphrase['view_join_requests_gcpuser'], sprintf($grouplink, $usergroupid))
			));
		}
	}
	print_table_footer();
...
if ($_REQUEST['do'] == 'reputation')
{
	...
	if (!can_moderate(0, 'caneditreputation'))
	{
		print_modcp_stop_message2('no_permission');
	}
	...
	$repcount = $vbulletin->db->query_first("
	...
	");
	if ($vbulletin->db->num_rows($comments))
	{

		if ($vbulletin->GPC['page'] != 1)
		{
			$prv = $vbulletin->GPC['page'] - 1;
			$firstpage = "<input type=\"button\" class=\"button\" value=\"&laquo; " . $vbphrase['first_page'] . "\" tabindex=\"1\" onclick=\"window.location='user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=reputation&u=" . $vbulletin->GPC['userid'] . "&pp=" . $vbulletin->GPC['perpage'] . "&page=1'\">";
			$prevpage = "<input type=\"button\" class=\"button\" value=\"&lt; " . $vbphrase['prev_page'] . "\" tabindex=\"1\" onclick=\"window.location='user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=reputation&u=" . $vbulletin->GPC['userid'] . "&pp=" . $vbulletin->GPC['perpage'] . "&page=$prv'\">";
		}

		if ($vbulletin->GPC['page'] != $totalpages)
		{
			$nxt = $vbulletin->GPC['page'] + 1;
			$nextpage = "<input type=\"button\" class=\"button\" value=\"" . $vbphrase['next_page'] . " &gt;\" tabindex=\"1\" onclick=\"window.location='user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=reputation&u=" . $vbulletin->GPC['userid'] . "&pp=" . $vbulletin->GPC['perpage'] . "&page=$nxt'\">";
			$lastpage = "<input type=\"button\" class=\"button\" value=\"" . $vbphrase['last_page'] . " &raquo;\" tabindex=\"1\" onclick=\"window.location='user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=reputation&u=" . $vbulletin->GPC['userid'] . "&pp=" . $vbulletin->GPC['perpage'] . "&page=$totalpages'\">";
		}
		...
		$headings = array();
		$headings[] = '<a href="user.php?' . vB::getCurrentSession()->get('sessionurl') . 'do=reputation&u=' . $vbulletin->GPC['userid'] . '&pp=' . $vbulletin->GPC['perpage'] . '&orderby=user&page=' . $vbulletin->GPC['page'] . "\" title='" . $vbphrase['order_by_username'] . "'>" . $vbphrase['username'] . "</a>";
		$headings[] = '<a href="user.php?' . vB::getCurrentSession()->get('sessionurl') . 'do=reputation&u=' . $vbulletin->GPC['userid'] . '&pp=' . $vbulletin->GPC['perpage'] . '&orderby=date&page=' . $vbulletin->GPC['page'] . "\" title='" . $vbphrase['order_by_date'] . "'>" . $vbphrase['date'] . "</a>";
		...
		{
			$cell = array();
			$cell[] = "<a href=\"user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=viewuser&amp;u=$comment[whoadded]\"><b>$comment[username]</b></a>";
			...
			$cell[] = construct_link_code($vbphrase['edit'], 'user.php?' . vB::getCurrentSession()->get('sessionurl') . "do=editreputation&reputationid=$comment[reputationid]");
			print_cells_row($cell);
		}
...
if ($_POST['do'] == 'doeditreputation')
{
	...
	$vbulletin->db->query_write("
		...
		SET reason = '" . $vbulletin->db->escape_string($vbulletin->GPC['reason']) . "'
		WHERE reputationid = " . $vbulletin->GPC['reputationid'] . "
	");

	print_modcp_stop_message2('updated_reason_successfully','user', array('do'=>'reputation','u' => $vbulletin->GPC['userid']));
