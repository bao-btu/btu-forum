...
* @date		$Date: 2010-02-09 18:29:22 -0200 (Tue, 09 Feb 2010) $
*/
class vB_DataManager_Event extends vB_DataManager
{
	...
	*/
	var $validfields = array(
		'eventid'       => array(vB_Cleaner::TYPE_UINT,      vB_DataManager_Constants::REQ_INCR, 'return ($data > 0);'),
		'userid'        => array(vB_Cleaner::TYPE_UINT,      vB_DataManager_Constants::REQ_YES,  vB_DataManager_Constants::VF_METHOD),
		'event'         => array(vB_Cleaner::TYPE_STR,       vB_DataManager_Constants::REQ_YES,  vB_DataManager_Constants::VF_METHOD),
		'title'         => array(vB_Cleaner::TYPE_STR,       vB_DataManager_Constants::REQ_YES,  vB_DataManager_Constants::VF_METHOD),
		'allowsmilies'  => array(vB_Cleaner::TYPE_UINT,      vB_DataManager_Constants::REQ_YES),
		'recurring'     => array(vB_Cleaner::TYPE_UINT,      vB_DataManager_Constants::REQ_YES),
		'recuroption'   => array(vB_Cleaner::TYPE_STR,       vB_DataManager_Constants::REQ_YES),
		'calendarid'    => array(vB_Cleaner::TYPE_UINT,      vB_DataManager_Constants::REQ_YES,  vB_DataManager_Constants::VF_METHOD),
		'customfields'  => array(vB_Cleaner::TYPE_ARRAY_STR, vB_DataManager_Constants::REQ_NO,   vB_DataManager_Constants::VF_METHOD, 'verify_serialized'),
		'visible'       => array(vB_Cleaner::TYPE_UINT,      vB_DataManager_Constants::REQ_NO),
		'utc'           => array(vB_Cleaner::TYPE_NUM,       vB_DataManager_Constants::REQ_YES,  vB_DataManager_Constants::VF_METHOD),
		'dst'           => array(vB_Cleaner::TYPE_UINT,      vB_DataManager_Constants::REQ_NO),
		'dateline'      => array(vB_Cleaner::TYPE_UNIXTIME,  vB_DataManager_Constants::REQ_AUTO),
		'dateline_from' => array(vB_Cleaner::TYPE_UINT,      vB_DataManager_Constants::REQ_YES),
		'dateline_to'   => array(vB_Cleaner::TYPE_UINT,      vB_DataManager_Constants::REQ_YES),
	);
	...
	*/
	function vB_DataManager_Event(&$registry, $errtype = vB_DataManager_Constants::ERRTYPE_STANDARD)
	{
		parent::vB_DataManager($registry, $errtype);

		// Legacy Hook 'eventdata_start' Removed //
	}
...
//		if ($this->dbobject->query_first("SELECT calendarid FROM " . TABLE_PREFIX . "calendar WHERE calendarid = $calendarid"))
		if (vB::getDbAssertor()->getField('calendar', array('calendarid'=>$calendarid)))
		{
			return true;
		}
		else
	...
	*/
	function set_userfields(&$userfields)
	{
		...
		");*/
		$customcalfields = $this->assertor->getRows('calendarcustomfield', array('calendarid'=>$calendarid), 'calendarcustomfieldid');
		$customfields = array();

		//while ($custom = $this->dbobject->fetch_array($customcalfields))
		foreach ($customcalfields AS  $custom)
		{
	...
	function pre_save($doquery = true)
	{
		...
		if ($this->verify_datetime)
		{
			...
			if ($this->info['type'] != 'single')
			{
				...
				if (($pos = strpos($this->options['timeformat'], 'H')) === false)
				{
					if ($to_ampm == 'PM')
					{
				...
				require_once(DIR . '/includes/functions_misc.php');
				$this->set_info('occurdate', vbgmdate('Y-n-j', $dateline_from + $this->userinfo['timezoneoffset'] * 3600, false, false));
			}
			...
			if ($this->info['type'] == 'recur')
			{
				$checkevent = array(
					...
					'dateline_from_user' => $dateline_from + $this->userinfo['timezoneoffset'] * 3600,
					'dateline_to_user'   => $dateline_to + $this->userinfo['timezoneoffset'] * 3600,
...
//				"))
				if ($query = $this->assertor->getField('event',
						array(
							'userid' => intval($this->fetch_field('userid')),
							'dateline_from' => intval($this->fetch_field('dateline_from')),
							'dateline_to' => intval($this->fetch_field('dateline_to')),
							'event' => $this->fetch_field('event'),
							'title' => $this->fetch_field('title'),
							'calendarid' => intval($this->fetch_field('calendarid'))
						)))
				{
		...
		$return_value = true;
		// Legacy Hook 'eventdata_presave' Removed //
	...
	*/
	function post_save_each($doquery = true)
	{
		...
		if ($this->condition AND ($this->fetch_field('dateline_from') - $this->existing['dateline_from']) >= 82800)
		{
			...
			");*/
			$this->assertor->update('suscribeevent', array('lastreminder'=>0), array('eventid'=>intval($this->fetch_field('eventid'))));
		}

		// Legacy Hook 'eventdata_postsave' Removed //
	...
	*/
	function post_delete($doquery = true)
	{
		...
		$this->assertor->delete('suscribeevent(', array('eventid'=>intval($this->fetch_field('eventid'))));

		require_once(DIR . '/includes/functions_calendar.php');
		build_events();

		// Legacy Hook 'eventdata_delete' Removed //
