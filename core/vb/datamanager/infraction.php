	...
	public function post_save_each($doquery = true)
	{
		...
		if (empty($this->existing))
		{
			...
			if ($userinfo = $this->info['userinfo'])
			{
				...
				$infractionChannel = $this->assertor->getRow('vBForum:channel', array('guid' => vB_Channel::INFRACTION_CHANNEL));
				...
				$channelinfo = $this->assertor->getRow('vBForum:channel',
					...
					'nodeid' => $infractionChannel['nodeid']));
				...
				$updates = array('title' => $subject,
					...
					'pagetext' => $message, 'parentid' => $infractionChannel['nodeid'],
