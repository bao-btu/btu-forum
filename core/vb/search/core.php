...
abstract class vB_Search_Core
{
	...
	public function getTwoPassResults(vB_Search_Criteria $criteria)
	{
		...
		if ($cacheKey)
		{
			...
			if ($cached !== false)
			{
				return $this->getSecondPassNodes($cached);
			}
			else
			{
				return $cacheKey;
	...
	public function getFromCache(vB_Search_Criteria $criteria, $search_json)
	{
		...
		array(
				...
				array(
						'field' => 'dateline',
						'value' => vB::getRequest()->getTimeNow() - vB_Api_Search::CACHE_TTL_SEC,
	...
	public static function saveSecondPassResults($results, $cacheKey)
	{
		$res = array();
		...
		return self::getSecondPassNodes($res);
	}

	/**
	 * Take first pass cached data and remove any data that doesn't match our selfonly permission
	 *
	 * @param array $cached
	 * @return string
	 */
	public static function getSecondPassNodes($cached)
	{
		$userdata = vB::getUserContext()->getAllChannelAccess();
		if (!empty($userdata['selfonly']) AND !empty($cached))
		{
			foreach ($cached AS $nodeid => $data)
			{
				if (in_array($data['parentid'], $userdata['selfonly']) AND $data['userid'] != vB::getUserContext()->fetchUserId())
				{
					unset($cached[$nodeid]);
				}
			}
