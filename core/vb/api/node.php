...
class vB_Api_Node extends vB_Api
{
	...
	**/
	...
	function getNodeAttachments($nodeids)
	{
		...
		foreach ($nodeids AS $nodeid)
		{
			if (!vB::getUserContext()->getChannelPermission('forumpermissions', 'cangetattachment', $nodeid))
			{
				return array();
			}
		}
	...
	public function listNodeFullContent($parentid, $page = 1, $perpage = 20, $depth = 0, $contenttypeid = null, $options = false)
	{
		...
		foreach ( $nodeList AS $key => $node)
		{
			...
			{
				$nodeList[$key]['ipaddress'] = '';
				$nodeList[$key]['content']['ipaddress'] = '';
			}
		}
	...
	*  							withParent => appends information from the parent. This info will append the 'parentConversation' info if the node is a comment.
	...
	public function getNodeFullContent($nodeid, $contenttypeid = false, $options = array())
	{
		...
		foreach ($result AS $key => $node)
		{
			...
			if (!$node['moderatorperms']['canviewips'])
			{
	...
	**/
	...
	public function fetchNodeStats($nodeid, $interval = self::DATE_RANGE_DAILY, $page = 1)
	{
		...
		// channel mods can stats
		$perms = vB_Api::instanceInternal('blog')->getChannelAdminPerms($nodeid);
		if (!vB::getUserContext()->getChannelPermission('moderatorpermissions', 'canmoderateposts', $nodeid) AND !$perms['canstats'])
		{
			throw new vB_Exception_Api('no_permission');
		}
	...
	*/
	...
	protected function addChannelLastData()
	{
		...
		foreach ($nodes AS $node)
		{
			if (!empty($channels[$node['nodeid']]))
			{
				...
				if (empty($channels[$node['nodeid']]['noDetail']))
				{
					...
					$channels[$node['nodeid']]['lastcontent'] = array ('nodeid' => $node['lastcontentid'],
						'authorname' => $node['lastcontentauthor'], 'userid' => $node['lastauthorid'], 'starter' => array(), 'created' => $node['lastcontent']);
	...
	protected function assembleTree(&$userTree, &$channels, $rootid, $depth, $userid)
	{
		...
		if (intval($userid))
		{
			$readData = vB::getDbAssertor()->assertQuery('noderead', array( vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT,
				'userid' => $userid, 'nodeid'  => array_keys($userTree)));
	...
	protected function addFinalBranches(&$trunk, &$tree, &$userTree, $maxDepth, $userid)
	{
		...
		if (isset($userTree[$trunk['nodeid']]) )
		{
		...
		if (!empty($trunk['lastcontent']) AND ($trunk['lastcontent']['created'] < $oldestNew))
		{
			$trunk['is_new'] = 0;
		}
		else if (!intval($userid) OR (empty($trunk['readtime']) OR (!empty($trunk['lastcontent']) AND ($trunk['lastcontent']['created'] > $trunk['readtime']))))
		{
			$trunk['is_new'] = 1;
		}
		else
	 ...
	 *
	 * @param mixed		array of all channel records
	 * @param mixed		array of channel records for the current user only
	 * @param mixed		a reference to the channel we are currently processing.
	 */
	protected function addChannelTotals(&$channels, &$userChannels, $channelid)
	{
		 ...
		 * parent channel may have both subchannels and direct totals.  So we keep a total four totals starting from bottom- textcount, totalcount,
		 * viewtotal. viewtext. The difference between textcount for the children and textcount for the parent is the number of posts directly on the parent.
		 *
		...
		if (!empty($channels[$channelid]['children']))
		{
			...
			{
				$textcount += $channels[$childid]['textcount'];
				$totalcount += $channels[$childid]['totalcount'];

				//We only add if the current user can see this.
				if (!empty($userChannels[$childid]))
				{
					if (!empty($userChannels[$childid]['noDetail']))
					{
						$userChannels[$childid]['posts'] = 0;
						$userChannels[$childid]['topics'] = 0;
					}
					else
					{
						$this->addChannelTotals($channels, $userChannels, $childid);
						$viewTotal += $userChannels[$childid]['posts'];
						$viewtext += $userChannels[$childid]['topics'];
					}
				}
			}
		}
		$userChannels[$channelid]['topics'] = $channels[$channelid]['textcount'] + $viewtext;
		$userChannels[$channelid]['posts'] = max($channels[$channelid]['totalcount'] + $viewTotal - $totalcount, 0);
	}
