...
class vB_Api_Widget extends vB_Api
{
	...
	public function fetchWidgets(array $widgetids = array())
	{
		...
		foreach ($widgets AS &$widget)
		{
			$widget['title'] = $vbphrases[$widget['template'] . '_widgettitle'];

			//provide a default in case the phrase isn't correctly defined.
			if (isset($phrases[$widget['category']]))
			{
				$widget['category_title'] = $vbphrases[$widget['category']];
			}
			else
			{
		...
		usort($widgets, array('vB_Api_Widget', 'compareWidgets'));
...
//			'widgetinstanceid' => $widgetinstanceid,
	...
	protected function _getWidgetDefinition($widgetid)
	{
		...
		// Fetch fields label and defaultvalue phrases
		$phrasestofetch = array();
		foreach ($fields as $field)
		{
			...
			// If defaultvalue isn't empty and starts with 'phrase:'
			if (!empty($field['defaultvalue']) AND !strncmp($field['defaultvalue'], 'phrase:', 7))
			{
				$phrasestofetch[] = substr($field['defaultvalue'], 7);
			}
			// If data is array, we should loop the first level to see if we need to convert string to phrase
			if (is_array($data))
			{
				foreach ($data as $k => $v)
				{
		...
		foreach ($fields as $field)
		{
			if (is_array($field) AND !empty($field))
			{
				...
				$configFields[$field['name']] = array(
					...
					'label' => $label,
					'type' => $field['field'],
					'defaultvalue' => (!empty($field['defaultvalue']) AND !strncmp($field['defaultvalue'], 'phrase:', 7)) ? $vbphrases[substr($field['defaultvalue'], 7)] : $field['defaultvalue'],
