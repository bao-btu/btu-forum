...
abstract class vB_Api_Content extends vB_Api
{
				...
				foreach ($nodes AS $node)
				{
					...
					if ($node['contenttypeid'] === vB_Types::instance()->getContentTypeId('vBForum_Channel'))
					{
						if ($userContext->getChannelPermission('forumpermissions2', 'candeletechannel', $node['nodeid']) OR
							$userContext->getChannelPermission('adminpermissions', 'canadminforums', $node['nodeid']) OR
	...
	*/
	...
	public function afterMove($nodeid, $oldparent, $newparent)
	{
		...
		$nodeAPI = vB_Api::instanceInternal('Search');
		foreach ($toUpdate AS $ancestor)
		{
			...
			$nodeAPI->attributeChanged($ancestor['nodeid']);
		}
	}
