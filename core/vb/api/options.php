...
class vB_Api_Options extends vB_Api
{
	...
	/**
	 * This function returns the options data for a specified list.
	 * @param mixed 	array of string
	...
	public static function fetchStatic($options = null)
	{
		...
		$dsOptions =  vB::getDatastore()->getValue('options');
		$response = array();
		foreach($options as $option)
		{
			if (isset($dsOptions[$option]))
			{
				if ($option == 'options')
				{
					$response['publicoptions'] = $dsOptions[$option];
				}
				else
				{
