...
//			'dosmilies'		=> $calendarinfo['allowsmilies']
	...
	*/
	...
	protected function doParseSignature($userid, $signature = false)
	{
		if (empty($signature))
		{
	...
	public function getSignatureInfo($userid, $signature = false)
	{
		...
		return $this->doParseSignature($userid, $signature);
	}
	
	/**
	 * Fetches and parses to html signatures
	 * @param array $userIds
	 * @param array $rawSignatures optionally pass the raw signatures to avoid fetching them again
	 * @return array the parsed (html) signatures
	 */
