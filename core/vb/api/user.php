...
class vB_Api_User extends vB_Api
{
	...
	protected $disableWhiteList = array('hasPermissions', 'fetchCurrentUserinfo', 'login');
	...
	public function fetchProfileInfo($userid = false)
	{
		...
		if (empty($userInfo))
		{
			...
			if (!empty($fields))
			{
				...
				$fieldsInfo = $assertor->assertQuery('vBForum:fetchCustomProfileFields', array('hidden' => $hidden));
	...
	*/
	...
	public function fetchCurrentUserinfo()
	{
		...
		if (!empty($userInfo['lang_options']) AND !is_array($userInfo['lang_options']))
		{
		...
		if(!empty($notCached))
		{
			...
			foreach ($avatars AS $user)
			{
				...
				else
				{
					...
					$defaultAvatarPath = 'images/default/default_avatar_' . $def_size . '.png';
...
//					}
	...
	public function save($userid, $password, $user, $options, $adminoptions, $userfield, $notificationOptions = array(), $hvinput = array(), $extra = array())
	{
		...
		if ($userid)
		{
			...
			// birthday
			if (!$userdata->adminoverride AND $user['birthday'] AND $olduser['birthday'] AND ($user['birthday'] != $olduser['birthday']) AND $vboptions['reqbirthday'])
			{
		...
		vB::getCurrentSession()->setSkipcache(true);
			...
			$maildata = vB_Api::instanceInternal('phrase')->
				fetchEmailPhrases('newuser',
					array(
						...
						vB5_Route::buildUrl('profile|nosession|fullurl', array('userid' => $user['userid'])),
						$user['email'],
						$user['birthday'],
						$user['ipaddress'],
						$customfields,
