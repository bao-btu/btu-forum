...
class vB_Api_Wol extends vB_Api
{
	...
	public function fetchAll($pagekey = '', $who = '', $pagenumber = 1, $perpage = 0, $sortfield = 'time', $sortorder = 'desc', $resolveIp = false)
	{
		...
		foreach ($allusers as $userRecord)
		{
			...
			if (($userRecord['userid'] > 0 AND (empty($onlineUsers[$userRecord['userid']]) OR $onlineUsers[$userRecord['userid']]['lastactivity'] < $userRecord['lastactivity']))
	...
	public function refreshUsers($pagekey = '', $who = '', $pagenumber = 1, $perpage = 0, $sortfield = 'time', $sortorder = 'desc', $resolveIp = false)
	{
		$result = array();
