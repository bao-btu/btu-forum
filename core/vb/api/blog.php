	...
	*/
	public function getBlogInfo($from = 1, $perpage = 20, $authorid = 0)
	{
		...
		foreach ($nodeContent['results'] AS $key => $node)
		{
			...
			$hashKey = 'vbRouteContentConversation_'. $node['nodeid'] . '_' . $authorid;
		...
		if (!empty($remaining_channelids))
		{
			...
			foreach ($routes as $record)
			{
				...
				$hashKey = 'vbRouteContentConversation_'. $record['contentid'] . '_' . $authorid;
				$cache->write($hashKey, $record, 1440, array('routeChg_' . $record['routeid'], 'nodeChg_' . $record['contentid']));
				if ($route AND ($pageKey = $route->getPageKey()))
				{
