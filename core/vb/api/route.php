	...
	public function getRoute($pathInfo, $queryString, $anchor = '')
	{
		...
		try
		{
			...
			if ($route)
			{
				...
				{
					return array(
						'redirect'	=> vB5_Route::buildUrl($redirectId, $route->getArguments(), $route->getQueryParameters(), $route->getAnchor()),
						'redirectRouteId' => $redirectId
					);
				}
				else if ($pathInfo != $canonicalPathInfo OR ($canonicalParam!== false AND $route->getQueryParameters()!=$canonicalParam))
				{
					return array(
