...
"
		...
		'getWidgetdefinition' => array(
			...
			'query_string' => "
				...
				WHERE widgetdefinition.widgetid = {widgetid}
			",
		),
		'fetchPhraseList' => array(
			vB_dB_Query::QUERYTYPE_KEY => vB_dB_Query::QUERY_SELECT,
			'query_string' => "SELECT l.languageid, p.varname, p.text, l.charset
					FROM {TABLE_PREFIX}phrase AS p
					LEFT JOIN {TABLE_PREFIX}language AS l ON p.languageid = l.languageid AND l.languageid IN ({languageid})
					WHERE p.varname IN ({varname}) ORDER BY l.languageid DESC
			",
		),
	...
	/* NOTE: If you uncomment this function, you need to fix the calling code
	to not pass the SQL 'condition' parameter, as it must be cleaned here.
	public function getIds($params, $db, $check_only = false)
	{
		...
		{
			$params = vB::getCleaner()->cleanArray($params, array(
				'condition' => vB_Cleaner::TYPE_NOCLEAN, // NEEDS TO BE cleaned in conditionsToFilter
				'table' => vB_Cleaner::TYPE_NOCLEAN, // cleaned with clean_identifier
				'join' => vB_Cleaner::TYPE_NOCLEAN, // cleaned with clean_identifier
			));

			$params['table'] = $db->clean_identifier($params['table']);
			$params['join'] = $db->clean_identifier($params['join']);
	...
	public function getBlogUserSummaryStats($params, $db, $check_only = false) {
		if ($check_only) {
			...
			$table = (!empty($params['userid']) ? "blog_user" : "blog_summary") . "stats";
	...
	public function getBlogMembers($params, $db, $check_only = false) {
		...
		else 
		{
			$params = vB::getCleaner()->cleanArray($params, array(
				...
				'avatarenabled' => vB_Cleaner::TYPE_NOCLEAN,
			));
		...
		else
		{
			...
			$sql = "SELECT title, templatetype, username, dateline, template_un AS template
						...
						WHERE styleid = " . $params['styleid'] . "
								AND templatetype = 'template' "
								. $params['templateids'] ? "AND templateid IN($params[templateids]) " : "" .
						"ORDER BY title";
	...
	public function userFind($params, $db, $check_only = false)
	{
		...
		else
		{
			...
			{
				$sql .= "email, parentemail, ";
			}
			$sql .= " (options & " . $bf_misc_useroptions['coppauser'] . ") AS coppauser,
...
// 				}
	...
	*/
	public function fetchPruneUsers($params, $db, $check_only = false)
	{
		...
		else
		{
			...
			$sqlcond = array();
			if ($cleaned['usergroupid'] != -1)
			{
				$sqlcond[] = "user.usergroupid = " . $cleaned['usergroupid'];
			}
			if ($cleaned['daysprune'])
			{
				$daysprune = intval(TIMENOW - $cleaned['daysprune'] * 86400);
				if ($daysprune < 0)
				{
			...
			if ($params['joindate']['month'] AND $params['joindate']['year'])
			{
				...
				$joindateunix = mktime(0, 0, 0, $cleaned['joindate']['month'], $cleaned['joindate']['day'], $cleaned['joindate']['year']);
				if ($joindateunix)
				{
			...
			if ($cleaned['minposts'])
			{
				$sqlcond[] = "posts < " . $cleaned['minposts'];
			}
			switch($params['order'])
			{
				case 'username':
	...
	*/
	 ...
	 *
	 **/
	protected function getNodePermTerms($excludeUserSpecific = false)
	{
		...
		else
		{
			...
			if (!empty($channelAccess['selfonly']))
			{
				...
				{	// result set is being pre-cached so include all nodes from selfonly.
					...
					$where[] = "(starter.parentid IN (" . implode(',', $channelAccess['selfonly']) . "))\n";
				}
			}
