...
class vB_Library_Style extends vB_Library
{
	...
	protected function addStylevars(&$styles)
	{
		...
		if (!empty($pending)) 
		{
			...
			if ($result) 
			{
				...
				{
					$this->setStylevars(intval($style['styleid']), $style['newstylevars']);
				}
			}	
		}
	...
	private function getStylevars($styleid)
	{
	...
	private function setStylevars($styleid, $data)
	{
		$styleid = intval($styleid);
	 ...
	 */
	public function setCssFileDate()
	{
		vB_Cache::instance()->event('vB_Library_Style_stylevar_cache');
		$options = vB::getDatastore()->getValue('miscoptions');
		$options['cssfiledate'] = vB::getRequest()->getTimeNow();
		vB::getDatastore()->build('miscoptions', serialize($options), 1);
	}
