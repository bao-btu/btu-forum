...
class vB_Library_Content_Video extends vB_Library_Content_Text
{
	...
	public function getFullContent($nodeid, $permissions = false)
	{
		...
		//If the result has a videocount we can skip the count query..
		$needVideos = array();
		foreach ($results AS $content)
		{
			if (!isset($content['videocount']))
			{
				$needVideos[] = $content['nodeid'];
			}
		}
		...
		$videoitems = $this->fetchVideoItems($needVideos);
		...
		else
		{
			...
			$maildata = vB_Api::instanceInternal('phrase')->
				fetchEmailPhrases('reply', array(
					...
					($currentNode['starter'] == $currentNode['parentid'] ? $phrase['thread_small'] : $phrase['post_small']),
