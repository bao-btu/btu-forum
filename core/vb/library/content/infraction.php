...
class vB_Library_Content_Infraction extends vB_Library_Content_Text
{
	...
	*/
	...
	public function add($data, array $options = array(), $convertWysiwygTextToBbcode = true)
	{
		...
		$result = array();
		$result['infractionNodeid'] = $nodeid;
		...
		if (empty($nodeid['errors']) AND !empty($data['message']))
		{
		...
		// invalidate cache
		$clearCacheNodeIds = array($data['parentid'], $data['infractednodeid']);
		if (empty($nodeid['errors']))
		{
	...
	protected function getInfractionPagetext(array $data, array $infractedNode, array $infractedUserInfo, array $infractionLevelInfo)
	{
		...
		if ($data['infractednodeid'])
		{
			...
			return sprintf(
				...
				vB5_Route::buildUrl($infractedNode['routeid'] . '|nosession|fullurl', array('nodeid' => $infractedNode['nodeid'], 'title' => $infractedNode['title']), array('p' => $infractedNode['nodeid'])) . '#post' . $infractedNode['nodeid'],
				// infracted topic title
				($infractedNode['nodeid'] == $infractedNode['starter'] ? $infractedNode['title'] : $infractedNode['startertitle']),
				// infracted user link
				vB5_Route::buildUrl('profile|nosession|fullurl', array('userid' => $infractedUserInfo['userid'])),
				// infracted user name
	...
	protected function sendPm(array $data, array $infractedNode, array $infractedUserInfo, array $infractionLevelInfo, $isWarning, $banToApply)
	{
		...
		$pmData = array(
			'sentto' => array($data['infracteduserid']),
		);
		...
		if(
			// they currently have a permanent ban, so don't notify them at all
		...
		$userCanReceiveEmail = (vB::getUserContext($infractedUserInfo['userid'])->hasPermission('genericpermissions', 'canemailmember') AND $vboptions['enableemail']);
		$userCanReceivePm = ($vboptions['enablepms'] AND vB::getUserContext($infractedUserInfo['userid'])->getUsergroupLimit('pmquota'));

		if($userCanReceivePm)
		{
			return vB_Api::instanceInternal('Content_Privatemessage')->add($pmData);
		}
		elseif($userCanReceiveEmail)
		{
			return vB_Mail::vbmail($infractedUserInfo['email'], $pmData['title'], $pmData['rawtext'], true);
		}
	...
	public function canInfractNode($nodeid, array $node = null)
	{
		...
		return (
			...
			$this->canInfractUser($node['userid'])
			// Cannot infract a post that already has an infraction
			AND empty($node['infraction'])
