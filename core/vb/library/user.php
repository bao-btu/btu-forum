...
class vB_Library_User extends vB_Library
{
	 ...
	 * $param	mixxed Currently only supports 'onlinestatus', but expandable
	...
	public function fetchUserWithPerms($userid, $languageid = 0, $nocache = false, $lastactivity = false)
	{
		...
		$fastCache->write("vB_UserGIT_$userid", $groupintopic, 5, array("userChg_$userid", "userPerms_$userid", 'perms_changed',  'vB_ChannelStructure_chg'));
	...
	*/
	...
	public function sendActivateEmail($userid)
	{
		...
		$maildata = vB_Api::instanceInternal('phrase')
			->fetchEmailPhrases('activateaccount', array($username, $vboptions['bbtitle'], $_SERVER['x-vb-presentation-base'], $userinfo['userid'], $activateid, $vboptions['webmasteremail']), array($username), $userinfo['languageid']);
	...
	public function preloadUserInfo($userids)
	{
		...
		if (!empty($cacheKeys))
		{
			...
			if (!empty($needLast))
			{
				...
				foreach($cacheKeys AS $userid => $cacheKey)
				{
					if (!empty($cached[$cacheKey]))
					{
						$this->fetchUserWithPerms($userid, $languageid, $lastData[$userid]);
		...
		if (!empty($cacheKeys))
		{
			...
			if ($gitQry->valid())
			{
				...
				{
					$git[$group['userid']][] = array('nodeid' => $group['nodeid'], 'groupid' => $group['groupid']);
				}
			}
			foreach($git AS $userid => $groups)
			{
				$fastCache->write("vB_UserGIT_$userid", $groups, 5, array("userChg_$userid", "userPerms_$userid", 'perms_changed',  'vB_ChannelStructure_chg'));
			}
