...
class vB_Library_Node extends vB_Library
{
	...
	**/
	...
	public function resetAncestorCounts($existing, $ancestorsData, $hard)
	{
		...
		foreach ($toUpdate AS $ancestor)
		{
			...
			$searchAPI->attributeChanged($ancestor['nodeid']);
		}
	...
	public function deleteNodes($deleteNodeIds, $hard, $reason, $ancestorsId, $starters, $modlog = true, $reportspam = false)
	{
		...
		else
		{
			...
			foreach ($nodes as $node)
			{
				...
				$result = vB::getDbAssertor()->assertQuery('vBForum:unPublishNode', array(
	...
	public function setPublishDate($nodeid, $datefrom, $dateto = null)
	{
		...
		$ret = $this->contentAPIs[$node['nodeid']]->update($nodeid, $data);
			...
			if (!empty($nodes))
			{
				...
				foreach ($nodes as $key => $nodeid)
				{
					foreach ($contentList AS $key2 => $content)
					{
						...
						if (!empty($contentList[$key2]['node_no_permission']))
						{
							unset($nodeList[$key2]);
							break;
						}
	...
	***/
	...
	function setApproved($approveNodeIds, $approved = true)
	{
		...
		foreach ($nodeIds AS $node)
		{
			...
			vB_Cache::allCacheEvent('nodeChg_' . $node);
			$thisNode = $this->getNode($node);
			if ($approved)
			{
				vB_Library_Content::getContentLib($thisNode['contenttypeid'])->incrementUserPostCount($thisNode);
			}
			else
			{
				vB_Library_Content::getContentLib($thisNode['contenttypeid'])->decrementUserPostCount($thisNode, 'unapprove');
			}
			$searchAPI->attributeChanged($thisNode);
		}
	...
	*/
	public function markRead($nodeid)
	{
		...
		/*
		 * automatic threadmarking has an error in fetching the parents which is causing VBV-10402. 
		 * This is disabled for now, meaning that the thread will have to be marked as read by hand...
		if ($threadMarking == 2)
		{
			...
			$parentData = vB::getDbAssertor()->assertQuery('vBForum:getParentLastRead', array('nodeid' => $nodeid,
					'userid' => $userid, 'canview' =>  array_merge($channelPerms['canview'], $channelPerms['canalwaysview'], $channelPerms['canmoderate'])));
			foreach ($parentData as $parent)
			{
				if (!empty($parent['readtime']) AND
					(($parent['lastcontent'] < $cutoff) OR ($parent['readtime'] > $parent['lastcontent']) OR ($parent['lastcontent'] = $node['publishdate']))
				)
				{
					$nodes_marked[] = $parent['parent'];
				}
			}
	...
	*/
	public function markChannelsRead($nodeid = 0)
	{
		...
		$toMarkQry = $assertor->assertQuery('getChannelsToMark', array('nodeid' => $nodeid, 'canview' => $canview,
			'cutoff' => $cutoff));
		...
		$assertor->assertQuery('channelsMarkRead', array(
				'nodeid' => $nodeid, 'userid' => $userid, 'readtime' => $timenow, 'canview' => $canview, 'cutoff' => $cutoff));
