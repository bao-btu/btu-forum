...
class vB_UserContext
{
	...
	public function reloadUserPerms($forceReload = false)
	{
		...
		else
		{
			...
			vB_Cache::allCacheEvent('userChg_' . $this->userid);
	...
	public function reloadGroupInTopic()
	{
		...
		$cached = vB_Cache::instance(vB_Cache::CACHE_FAST)->read("vB_UserGIT_" . $this->userid);
		//We need to query the database.
		if ($cached !== false)
		{
			foreach ($cached as $perms)
			{
				$this->groupInTopic[$perms['nodeid']][$perms['groupid']] = $perms['groupid'];
			}
			return;
		}
		...
		foreach ($groupInTopic as $topic)
		{
			...
			$toCache[] = array('nodeid' => $topic['nodeid'], 'groupid' => $topic['groupid']);
		}
		vB_Cache::instance(vB_Cache::CACHE_FAST)->write("vB_UserGIT_"  . $this->userid, $toCache, 3600,
			array("userChg_" . $this->userid, "userPerms_" . $this->userid, 'perms_changed',  'vB_ChannelStructure_chg'));
	}
		...
		foreach($channels AS $channel)
		{
			...
			else
			{
				...
				$result[$channel] = array(
					'canview'				=> (int)$this->getChannelPermission('forumpermissions', 'canview', $channel),
