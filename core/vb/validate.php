...
abstract class vB_Validate
{
	...
	{
		if (!vB_Library::instance('Style')->validStyle($value))
		{
	...
	public static function stringLength($value, &$error, $minlength = 0, $maxlength = 0)
	{
		...
		if ($maxlength AND ($length = vB_String::vbStrlen($value) > $maxlength))
		{
			$max_error = true;
		}

		if ($minlength AND ($length = vB_String::vbStrlen($value)) < $minlength)
		{
			$min_error = true;
	...
	public static function username($value, &$error)
	{
		...
		"))
		{
			...
			$this->error('usernametaken', htmlspecialchars_uni($username), vB::getCurrentSession()->get('sessionurl'));
			return false;
		}
		...
		else if (!empty($this->registry->options['usernameregex']))
		{
	...
	*/
	public static function verify_list(&$list, $glue = ',', $dropzero = false)
	{
		if ($list !== '')
		{
			...
			$list = array_unique($this->registry->cleaner->clean($list, vB_Cleaner::TYPE_ARRAY_INT));
			sort($list);
