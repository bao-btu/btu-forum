...
class vB_ProfileCustomize
{
	...
	* in getDefaultTheme() below, and in /clientscript/vbulletin_userprofile.js **/
	...
	public static function getThemes()
	{
		if (!self::$themes)
		{
			$theme_data = vB_dB_Assertor::instance()->assertQuery('customprofile',
				array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT, 'userid' => '0'), 'title');
			self::$themes = array();
	...
	***/
	...
	public static function getSiteDefaultTheme()
	{
		...
		$theme_data = vB_dB_Assertor::instance()->assertQuery('customprofile',
				array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT, 'userid' => -1) );
		if ($theme_data->valid())
		{
			$theme = $theme_data->current();
			if ($theme_data->valid())
	...
	**/
	...
	public static function setPermissions($permissions = false)
	{
		...
		self::$datastore = vB::getDatastore();
		$cssPermissions = self::$datastore->get_value('bf_ugp_usercsspermissions');
		self::$permissions['cancustomize'] = $permissions & $cssPermissions['cancustomize'];
		self::$permissions['caneditfontfamily'] = $permissions & $cssPermissions['caneditfontfamily'];
		self::$permissions['caneditfontsize'] = $permissions & $cssPermissions['caneditfontsize'];
		self::$permissions['caneditbgimage'] = $permissions & $cssPermissions['caneditbgimage'];
		self::$permissions['caneditcolors'] = $permissions & $cssPermissions['caneditcolors'];
		self::$permissions['caneditborders'] = $permissions & $cssPermissions['caneditborders'];
	}
	...
	***/
	public static function getUserTheme($userid)
	{
		...
		$options = self::$datastore->get_value('options');
		$enabled = $options['enable_profile_styling'];
		$cancustomize = self::$permissions['cancustomize'];

		if (!$enabled OR !isset($userid) OR !$cancustomize)
		{
		...
		$theme_data = vB_dB_Assertor::instance()->assertQuery('customprofile',
			array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT, 'userid' => $userid));
		if (!$theme_data->valid())
		{
			$theme_data = vB_dB_Assertor::instance()->assertQuery('customprofile',
				array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT, 'userid' => -1));
	...
	{
		$vars = array(
			'font_family' => vB_Cleaner::TYPE_STR,
			'fontsize' => vB_Cleaner::TYPE_STR,
			'title_text_color' => vB_Cleaner::TYPE_STR,
			'page_background_color' => vB_Cleaner::TYPE_STR,
			'page_background_image' => vB_Cleaner::TYPE_STR,
			'page_background_image' => vB_Cleaner::TYPE_STR,
			'page_background_repeat' => vB_Cleaner::TYPE_STR,
			'module_text_color' => vB_Cleaner::TYPE_STR,
			'module_link_color' => vB_Cleaner::TYPE_STR,
			'module_background_color' => vB_Cleaner::TYPE_STR,
			'module_background_image' => vB_Cleaner::TYPE_STR,
			'module_background_repeat' => vB_Cleaner::TYPE_STR,
			'module_border' => vB_Cleaner::TYPE_STR,
			'moduleinactive_text_color' => vB_Cleaner::TYPE_STR,
			'moduleinactive_link_color' => vB_Cleaner::TYPE_STR,
			'moduleinactive_background_color' => vB_Cleaner::TYPE_STR,
			'moduleinactive_background_image' => vB_Cleaner::TYPE_STR,
			'moduleinactive_background_repeat' => vB_Cleaner::TYPE_STR,
			'moduleinactive_border' => vB_Cleaner::TYPE_STR,
			'headers_text_color' => vB_Cleaner::TYPE_STR,
			'headers_link_color' => vB_Cleaner::TYPE_STR,
			'headers_background_color' => vB_Cleaner::TYPE_STR,
			'headers_background_image' => vB_Cleaner::TYPE_STR,
			'headers_background_repeat' => vB_Cleaner::TYPE_STR,
			'headers_border' => vB_Cleaner::TYPE_STR,
			'content_text_color' => vB_Cleaner::TYPE_STR,
			'content_link_color' => vB_Cleaner::TYPE_STR,
			'content_background_color' => vB_Cleaner::TYPE_STR,
			'content_background_image' => vB_Cleaner::TYPE_STR,
			'content_background_repeat' => vB_Cleaner::TYPE_STR,
			'content_border' => vB_Cleaner::TYPE_STR,
			'button_text_color' => vB_Cleaner::TYPE_STR,
			'button_background_color' => vB_Cleaner::TYPE_STR,
			'button_background_image' => vB_Cleaner::TYPE_STR,
			'button_background_repeat' => vB_Cleaner::TYPE_STR,
			'button_border' => vB_Cleaner::TYPE_STR,
			'page_link_color' => vB_Cleaner::TYPE_STR);
		...
		$vars = array('themeid' => vB_Cleaner::TYPE_UINT,
			'deletetheme' => vB_Cleaner::TYPE_UINT,
			'saveasdefault' => vB_Cleaner::TYPE_UINT);
		...
		$savedprofile = vB_dB_Assertor::instance()->assertQuery('customprofile',
			array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT, 'userid' => $userid));
			...
			AND $current)
		{
			...
			$response =  vB_dB_Assertor::instance()->assertQuery('customprofile',
				array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_DELETE, 'customprofileid' => $current['customprofileid']));
	...
	public static function getAlbums($userinfo)
	{
		...
		//execute the query
		$albums = vB_dB_Assertor::instance()->assertQuery('CustomProfileAlbums',
	...
	public static function getAlbumContents($albumid, $userinfo)
	{
		...
		//do the query
		$attachments = vB_dB_Assertor::instance()->assertQuery('GetAlbumContents',
		array(vB_dB_Query::TYPE_KEY  => vB_dB_Query::QUERY_STORED,
	...
	public static function getAssetPicker($userinfo, $registry)
	{
		...
		$albums = vB_dB_Assertor::instance()->assertQuery('CustomProfileAlbums',
			array(vB_dB_Query::TYPE_KEY  => vB_dB_Query::QUERY_STORED, 'userid' => $userinfo['userid']));
		$album = $albums->current();
		$album_select = '';
		while ($albums->valid())
		{
