...
class vB_Types
{
	...
	protected function getTypeInfo()
	{
		...
		$query = vB::getDbAssertor()->assertQuery('vBForum:getContentTypes', array(vB_dB_Query::TYPE_KEY => vB_Db_Query::QUERY_STORED));
		$types = array();
		$type = $query->current();
		while ($query->valid())
		{
		...
		if (empty($types) AND (VB_ENTRY != 'upgrade.php'))
		{
	...
	protected function loadPackages($type_info)
	{
		...
		//If we are upgrading from a vB_3 database  there is no contenttype table yet.
		if (!sizeof($this->packages) AND (VB_ENTRY != 'upgrade.php'))
		{
	...
	protected function loadContentTypes($type_info)
	{
		...
		//If we are upgrading from a vB_3 database  there is no contenttype table yet.
		if (!sizeof($this->contenttypes) AND (VB_ENTRY != 'upgrade.php'))
		{
